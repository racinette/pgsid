  ...
  switch (tag) {
    ...
    case 'TypeCast': {
      ...
      if (!argIsLiteral) {
        ...
        if (!source.every((s) => s !== UNKNOWN_TYPE && catalog.isBuiltinImmutableIoRendering(s))) {
          return null
        }
      }
    ...
    case 'A_Expr': {
      ...
      if (kind === 'AEXPR_OP_ANY' || kind === 'AEXPR_OP_ALL') {
        ...
        return catalog.closedOperatorTypes(op, leftSet, elements) === null ? null : ['boolean']
      }
      if (kind === 'AEXPR_IN') {
      ...
      if (kind === 'AEXPR_DISTINCT' || kind === 'AEXPR_NOT_DISTINCT') {
        if (leftSet === undefined || rightSets.length !== 1) return null
        return catalog.closedOperatorTypes(op, leftSet, rightSets[0]!) === null ? null : ['boolean']
      }
      // AEXPR_OP / LIKE / ILIKE — the survivors' union is the node's set.
      if (rightSets.length !== 1) return null
      return catalog.closedOperatorTypes(op, leftSet === undefined ? null : leftSet, rightSets[0]!)
    }
...
function closedLimitClause(
  ...
  if (
    option !== undefined &&
    option !== 'LIMIT_OPTION_DEFAULT' &&
    option !== 'LIMIT_OPTION_COUNT'
  ) {
    return false
  }
 ...
 * noise.
 */
export function collectClosedSubtrees(root: Node, catalog: SubtreeEvaluationCatalog): Node[] {
