...
// *distinctness* ('a' ≠ 'b') is derivable only under the catalog gate
// (`literalDistinctnessSound`: enum identity, or text-family identity under a
// proven-deterministic collation). Under a nondeterministic collation,
// differently-spelled strings can be equal, and the refusal keeps negative
// branches of a discriminated CHECK safely unprovable there.
//
 ...
 * over-keep direction.
 */
function shapesDisjoint(sw: number, rel: 'lt' | 'eq' | 'gt' | 'ne', sq: number): boolean {
 ...
 */
function shapesContained(sw: number, rel: 'lt' | 'eq' | 'gt' | 'ne', sq: number): boolean {
 ...
 */
export function scanLitComparisons(expr: Node): { column: string; op: string; lit: Lit }[] {
  const out: { column: string; op: string; lit: Lit }[] = []
  const lastColumnName = (n: unknown): string | null => {
    const fields = (
      (n as Record<string, unknown>)?.['ColumnRef'] as { fields?: Node[] } | undefined
    )?.fields
    if (!Array.isArray(fields) || fields.length === 0) return null
    return (fields[fields.length - 1] as { String?: { sval?: string } })?.String?.sval ?? null
  }
...
class EntailmentKernel {
  ...
  runGuardTruth(guard: Node): boolean | undefined {
    ...
    if (this.isTrue(guard)) {
      this.input.trace?.addFact(
        'guardProven',
        'the facts prove the guard TRUE on every emitted row',
      )
      return true
    }
    return undefined
   ...
   */
  private armCondition(ce: { arg?: Node }, when: { expr?: Node } | undefined): Node | undefined {
  ...
  private selectedArmCondition(eq: { column: string; expr: Node }, lit: Lit): Node | null {
   ...
   */
  private elseSelectedConditions(eq: { column: string; expr: Node }, lit: Lit): Node[] | null {
    const node = eq.expr as Record<string, unknown>
    const ce = node['CaseExpr'] as { arg?: Node; args?: Node[]; defresult?: Node } | undefined
    if (!ce) return null
  ...
  private litColumnTyped(colKey: string, lit: Lit): boolean {
    if (lit.cast === null) return true
    const colType = this.colTypeRef(colKey)
    return colType !== null && lit.cast.name === colType.name && lit.cast.schema === colType.schema
  }
