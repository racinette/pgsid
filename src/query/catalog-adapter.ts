...
> {
  ...
  const notNullDomainOids = new Set(snapshot.domains.filter((d) => d.notNull).map((d) => d.oid))
  const domainForcedNotNull = (c: { typeOid: number }): boolean => notNullDomainOids.has(c.typeOid)
  ...
  for (const t of snapshot.tables) {
    ...
    for (const con of t.constraints) {
      ...
      try {
        ...
        const constraint = (
          alter?.cmds?.[0]?.AlterTableCmd?.def as
            { Constraint?: { contype?: string; raw_expr?: Node } } | undefined
        )?.Constraint
  ...
  for (const t of snapshot.tables) {
    for (const c of t.columns) {
      columnCollationDeterministic.set(`${t.schema}.${t.name}.${c.name}`, c.collationDeterministic)
      columnCollationIsDefault.set(`${t.schema}.${t.name}.${c.name}`, c.collationIsDefault ?? null)
    }
  }
  ...
  const resolveColumnCollationIsDefault = (
    ...
    const key = `${schema}.${table}.${column}`
    return columnCollationIsDefault.has(key) ? (columnCollationIsDefault.get(key) ?? null) : false
  }
  ...
  const resolveFunctions = (schema: string | undefined, name: string): ResolvedFunction[] => {
  ...
  const resolveColumnNotNull = (schema: string, table: string, column: string): boolean => {
  ...
  const resolveColumnNotNullTree = (schema: string, table: string, column: string): boolean => {
    const t = tableMap.get(`${schema}.${table}`)
    if (!t) return false
    return t.notNullTreeCols.has(column)
  }

  const resolveColumnTypeOid = (schema: string, table: string, column: string): number | null => {
    const t = tableMap.get(`${schema}.${table}`)
    return t?.colTypeOids.get(column) ?? null
  }

  const resolveColumnTypeName = (schema: string, table: string, column: string): string | null => {
    const t = tableMap.get(`${schema}.${table}`)
  ...
  const resolveFunctionMetadata = (schema: string | undefined, name: string) => {
  ...
  const isNotNullDomainByName = (schema: string | undefined, typeName: string): boolean => {
    ...
    // best answer.
    return (
      (schema ? domainNames.get(`${schema}.${typeName}`) : inPath(domainNames, typeName)) ?? false
    )
  }
  ...
  const resolveDomainBaseTypeName = (schema: string | undefined, typeName: string): string | null =>
  ...
  const resolveOperatorMetadata = (
    ...
    const narrowed =
      leftTypes === undefined && rightTypes === undefined ? candidates : candidates.filter(survives)
  ...
  const resolveBuiltinFunctionSignatures = (
    ...
    schema === undefined || schema === 'pg_catalog' ? (builtinFnSigsByName.get(name) ?? []) : []
  ...
  const kindOfRendered = (rendered: string): string | null => {
    ...
    if (qualified ? enumTypeNames.has(rendered) : inPath(enumTypeNames, rendered) !== undefined)
      return 'e'
    if (
      qualified ? compositeTypes.has(rendered) : resolveCompositeType(undefined, rendered) !== null
    )
      return 'c'
  ...
  const resolveOperatorTotality = (
    ...
    const builtins = resolveBuiltinOperatorSignatures(schema, name).filter(
      (o) => o.leftType !== null && o.rightType !== null,
    )
    ...
    const survives = (l: string, r: string): boolean => reaches(Ls, l) && reaches(Rs, r)
  ...
  const resolveUnaryOperatorTotality = (
    ...
    const userSurvivors = users.filter((o) => As.some((m) => mayCoerceImplicitly(m, o.rightType!)))
  ...
  const resolveUserFunctionTyped = (
    ...
    const builtinSurvivors = bRows.filter((r) => sets.every((s, i) => reaches(s, bParamAt(r, i))))
    const winner =
      userSurvivors.length === 1 && builtinSurvivors.length === 0 ? userSurvivors[0]! : null
  ...
  const resolveBuiltinScalarTotality = (
    ...
    const verdictOf = (
      r: BuiltinFunctionSignature,
    ): 'always' | 'first-arg' | 'strict-total' | null => {
    ...
    if (verdicts.every((v) => v === 'always' || v === 'first-arg'))
      return { kind: 'first-arg', returns }
  ...
  const resolveBuiltinFunctionShape = (schema: string | undefined, name: string): string | null =>
    schema === undefined || schema === 'pg_catalog' ? (builtinTableFunctions[name] ?? null) : null
  ...
  const immutableIoRendered = new Set([...immutableIoTypes].map((t) => typeAliases[t] ?? t))

  const UNKNOWN_TYPE = 'unknown'
  const isUnknownOperand = (s: readonly string[]): boolean =>
    s.length === 1 && s[0] === UNKNOWN_TYPE
  const singletonOf = (s: readonly string[]): string | null => (s.length === 1 ? s[0]! : null)
  ...
  const closedOperatorTypes = (
    ...
    const pool = [...(opVolatilities.get(name) ?? []), ...userOperatorVolatilityRows(name)].filter(
      (o) =>
    ...
    if (assumedR !== null && (unary || assumedL !== null)) {
      const exact = pool.find((o) => o.rightType === assumedR && (unary || o.leftType === assumedL))
  ...
  const closedFunctionTypesOfKind = (
    ...
    const exactAtKnowns = pool.filter((r) =>
      argTypes.every((s, i) => isUnknownOperand(s) || (s.length === 1 && paramAt(r, i) === s[0])),
    )
    ...
    const survivors = pool.filter((r) =>
      argTypes.every(
        (s, i) => isUnknownOperand(s) || s.some((m) => mayCoerceImplicitly(m, paramAt(r, i))),
      ),
    )
   ...
   */
  const closedCommonTypes = (memberTypes: readonly (readonly string[])[]): string[] | null => {
  ...
  // enum_in, and widening that is not first-wave scope.
  {
    ...
    const typeNameAstOf = async (rendered: string): Promise<unknown | null> => {
      ...
      try {
        ...
        const target = (
          (parsed.stmts?.[0]?.stmt as Record<string, unknown> | undefined)?.['SelectStmt'] as
            { targetList?: { ResTarget?: { val?: Record<string, unknown> } }[] } | undefined
    ...
    const substituteValue = (body: unknown, typeName: unknown): unknown => {
      const isValueRef = (n: unknown): boolean => {
        const fields = (
          (n as Record<string, unknown>)?.['ColumnRef'] as { fields?: unknown[] } | undefined
        )?.fields
        return (
...
async function parseExprAst(expr: string): Promise<Node | null> {
  try {
    ...
    const stmt = parsed.stmts?.[0]?.stmt as
      { SelectStmt?: { targetList?: { ResTarget?: { val?: Node } }[] } } | undefined
    ...
    try {
      ...
      if (stmts.length > 0) {
        const cf = (stmts[0]!.stmt! as Record<string, unknown>)['CreateFunctionStmt'] as
          { sql_body?: { List?: { items?: Node[] } } } | undefined
        if (cf?.sql_body) {
