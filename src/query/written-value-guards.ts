...
function shapeOf(stmt: Node): DmlShape | null {
  ...
  for (const [kind, key] of [
    ...
    const returningList = ((s['returningClause'] as { exprs?: Node[] } | undefined)?.exprs ??
      []) as Node[]
...
function setPairs(
...
): (readonly [string | undefined, Node | undefined])[] {
  return (targetList ?? []).map((item) => {
    ...
    const val = rt?.val && 'MultiAssignRef' in (rt.val as Fields) ? undefined : rt?.val
...
function guardsIn(returningList: readonly Node[]): Node[] {
  ...
  const visit = (n: unknown): void => {
    ...
    const ce = (n as Fields)['CaseExpr'] as { arg?: Node; args?: Node[] } | undefined
    if (ce && !ce.arg) {
...
function substitute(
  ...
  const walk = (n: unknown): unknown => {
    ...
    if (cr) {
      ...
      const addressed = qualifier === null ? allowUnqualified : targetNames.has(qualifier)
