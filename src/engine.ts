...
type MigrationFilesystemEvent =
  MigrationAddedFilesystemEvent | MigrationModifiedFilesystemEvent | MigrationDeletedFilesystemEvent
...
type IngestedEvent =
  MigrationFilesystemEvent | QueryFilesystemEvent | MigrationsReadyFilesystemEvent
...
class MigrationTracker {
  ...
  private pendingEvent:
    MigrationModifiedFilesystemEvent | MigrationAddedFilesystemEvent | SyntheticRetryEvent | null =
    null
  ...
  private _setAndNotify(
    event: MigrationModifiedFilesystemEvent | MigrationAddedFilesystemEvent | SyntheticRetryEvent,
  ) {
  ...
  pushEvent(event: MigrationModifiedFilesystemEvent | SyntheticRetryEvent) {
    ...
    if (pendingEvent !== null) {
      ...
      if (event.ts > lastProcessedEvent.ts) {
        if (this.lastFilesystemReadAt !== null) {
          if (event.ts > this.lastFilesystemReadAt) {
            ...
            this.logger.debug(
              'NOTICE: Received an event, but the event is older than the latest filesystem read, ignoring',
              {
                event,
        ...
        this.logger.debug(
          'NOTICE: Received an event, but the last processed event is newer (pending is not set), ignoring',
          {
            event,
            lastProcessedEvent,
          },
      ...
      this.logger.debug(
        'NOTICE: Received an event, but there is no pending event and no last processed one, ignoring',
        {
          event,
        },
      )
