...
function typedCall<M extends CallableMetadata>(helper: string): CallableEmitter<M, GoExpression> {
  return {
    helpers: [helper],
    emit: (metadata, operands) => ({
      type: metadata.result,
      expression: go.call(
        go.ident(helper),
        operands.flatMap((operand) => [go.string(operand.type), operand.expression]),
      ),
    }),
  }
}

export const goJsonOperators = {
...
} satisfies OperatorBindings<typeof PG18_JSON, GoExpression>

export const goJsonFunctions = {
...
} satisfies FunctionBindings<typeof PG18_JSON, GoExpression>
