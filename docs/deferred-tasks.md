...
| `A_Indirection` over a CLOSED array | nullable | Found 2026-08-24 while closing the row above. `(array_remove(ARRAY['a','b'], 'a'))[1]` is non-NULL on every row and the engine claims nothing: the ARGUMENT closes now, but `A_Indirection` is not in the evaluator's closed grammar at all, so the subscript around it is open whatever it contains. Distinct from the permanent `A_Indirection` row above, which is about a subscript over a COLUMN — this one is a grammar gap, not a shape the walk declines to reason about. The symbolic rule that covers `(ARRAY['a','b'])[1]` reads the constructor's own length and has nothing to read here. Boundary test: `polymorphic-landing-red.test.ts`, "a SUBSCRIPT over the same call is refused" |
