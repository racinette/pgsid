...
| **A CTE column the re-export reading cannot type** | **open, located, and now the whole cost of the two rows above** | The two refusals are at the NAME-LEVEL fallback, which is reached only where the operand types are unreadable — so the price of the fix is exactly the set of shapes that read as unreadable and did not deserve to. Measured 2026-08-24 through the type-set audit: it is not "everything behind a CTE", it is **two shapes, one function**. `reExportedBaseColumn` (nullability-walk.ts) follows a CTE/subquery column to a BASE TABLE column and refuses everything else — a target list under `SETOP_*` on its first line, a target entry that is not a bare `ColumnRef` on its `cr` check. A PLAIN CTE column types today and still claims. A set-op column reads UNKNOWN **whatever its branches are** — `s.n` as much as `s.a`, which is also why `s.n + 1` claims via the LITERAL and not via the union, correcting what that fixture's own comment used to say. A computed column (`r.n * 2`) reads UNKNOWN even though the walk typed the expression while analysing the body and then dropped the reading at the re-export. Both are pinned RED in `half-known-operands.test.ts`, "the measured COST of the refusal". The fix is not a new mechanism: `elementTypeInSelect` already builds an inner scope with the walk's own `buildScope` and evaluates an expression in it, guarding `UnsupportedNodeError` — the same shape applied at those two refusal points, plus a `seen` guard for WITH RECURSIVE. This is the retirement condition `catalog-adapter.ts` recorded for the name-rule fallback ("the fallback retires when those two sources type"), now measured rather than assumed |
