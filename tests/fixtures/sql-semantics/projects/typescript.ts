...
function decimalInput(text: string | null): SqlDecimal | null {
    if (text === null)
        return null;
    const match = /^(?:[+-]?)(?:\d*)(?:\.(\d*))?(?:[eE]([+-]?\d+))?$/.exec(text);
    const exponent = Number(match?.[2] ?? 0);
    if (!Number.isSafeInteger(exponent) || Math.abs(exponent) > 1073741823)
        throw new SqlIntegerError();
    ...
    if (match && !value.isFinite())
        throw new SqlIntegerError();
    return new SqlDecimal(value, value.isFinite() ? scale : 0);
}
function sqlDecimalContext(left: Decimal, right: Decimal): typeof Decimal {
    const precision = left.isFinite() && right.isFinite() ? Math.max(1, Math.max(left.e, right.e) + Math.max(left.dp(), right.dp()) + 3, left.sd() + right.sd() + 3) : 34;
    return Decimal.clone({ precision, rounding: Decimal.ROUND_HALF_UP, modulo: Decimal.ROUND_DOWN, minE: -1000000000, maxE: 1000000000 });
}
function sqlDecimalWeight(value: Decimal): {
    weight: number;
    first: number;
} {
    if (value.isZero())
...
function sqlDecimalBinary(left: SqlDecimal | null, right: SqlDecimal | null, op: string): SqlDecimal | null {
    if (left === null || right === null)
        return null;
    const a = left.value, b = right.value;
    if (a.isNaN() || b.isNaN())
        return new SqlDecimal(new Decimal(NaN), 0);
    if (["div", "quotient", "mod"].includes(op) && b.isZero())
        throw new SqlDivisionByZeroError();
    if (op === "mod" && !b.isFinite() && a.isFinite())
        return left;
    let scale = Math.max(left.scale, right.scale);
    let Constructor = sqlDecimalContext(a, b);
    if (op === "mul")
        scale = Math.min(16383, left.scale + right.scale);
    ...
    switch (op) {
        ...
        case "sub":
            ...
            break;
        case "mul":
            value = x.mul(y).toDP(scale, Decimal.ROUND_HALF_UP);
            break;
        case "div":
            value = x.div(y).toDP(scale, Decimal.ROUND_HALF_UP);
            break;
        case "quotient":
            value = x.div(y).trunc();
            scale = 0;
...
function decimalAdd(left: SqlDecimal | null, right: SqlDecimal | null): SqlDecimal | null { return sqlDecimalBinary(left, right, "add"); }
function decimalSub(left: SqlDecimal | null, right: SqlDecimal | null): SqlDecimal | null { return sqlDecimalBinary(left, right, "sub"); }
function decimalMul(left: SqlDecimal | null, right: SqlDecimal | null): SqlDecimal | null { return sqlDecimalBinary(left, right, "mul"); }
function decimalMod(left: SqlDecimal | null, right: SqlDecimal | null): SqlDecimal | null { return sqlDecimalBinary(left, right, "mod"); }
function decimalDiv(left: SqlDecimal | null, right: SqlDecimal | null): SqlDecimal | null { return sqlDecimalBinary(left, right, "div"); }
function sqlDecimalRound(value: SqlDecimal | null, digits: bigint | null, truncate: boolean): SqlDecimal | null {
...
function decimalRound(value: SqlDecimal | null, scale: bigint | null = 0n): SqlDecimal | null { return sqlDecimalRound(value, scale, false); }
function sqlDecimalCompare(left: Decimal, right: Decimal): number {
    if (left.isNaN())
...
function decimalEq(left: SqlDecimal | null, right: SqlDecimal | null): boolean | null { return left === null || right === null ? null : sqlDecimalCompare(left.value, right.value) === 0; }
function decimalNe(left: SqlDecimal | null, right: SqlDecimal | null): boolean | null { return left === null || right === null ? null : sqlDecimalCompare(left.value, right.value) !== 0; }
function decimalLt(left: SqlDecimal | null, right: SqlDecimal | null): boolean | null { return left === null || right === null ? null : sqlDecimalCompare(left.value, right.value) < 0; }
function decimalLe(left: SqlDecimal | null, right: SqlDecimal | null): boolean | null { return left === null || right === null ? null : sqlDecimalCompare(left.value, right.value) <= 0; }
function decimalGt(left: SqlDecimal | null, right: SqlDecimal | null): boolean | null { return left === null || right === null ? null : sqlDecimalCompare(left.value, right.value) > 0; }
function decimalGe(left: SqlDecimal | null, right: SqlDecimal | null): boolean | null { return left === null || right === null ? null : sqlDecimalCompare(left.value, right.value) >= 0; }
function decimalIdentity(value: SqlDecimal | null): SqlDecimal | null { return value === null ? null : new SqlDecimal(value.value, value.scale); }
function decimalNeg(value: SqlDecimal | null): SqlDecimal | null { return value === null ? null : new SqlDecimal(value.value.neg(), value.scale); }
function decimalAbs(value: SqlDecimal | null): SqlDecimal | null { return value === null ? null : new SqlDecimal(value.value.abs(), value.scale); }
function decimalCeil(value: SqlDecimal | null): SqlDecimal | null { return value === null ? null : new SqlDecimal(value.value.ceil(), 0); }
function decimalFloor(value: SqlDecimal | null): SqlDecimal | null { return value === null ? null : new SqlDecimal(value.value.floor(), 0); }
function decimalSign(value: SqlDecimal | null): SqlDecimal | null { return value === null ? null : new SqlDecimal(new Decimal(value.value.isNaN() ? NaN : value.value.isZero() ? 0 : value.value.isNegative() ? -1 : 1), 0); }
function decimalTrunc(value: SqlDecimal | null, scale: bigint | null = 0n): SqlDecimal | null { return sqlDecimalRound(value, scale, true); }
function decimalQuotient(left: SqlDecimal | null, right: SqlDecimal | null): SqlDecimal | null { return sqlDecimalBinary(left, right, "quotient"); }
function sqlDecimalGcd(left: SqlDecimal | null, right: SqlDecimal | null, lcm: boolean): SqlDecimal | null {
    if (left === null || right === null)
        return null;
    if (!left.value.isFinite() || !right.value.isFinite())
        return new SqlDecimal(new Decimal(NaN), 0);
...
function decimalGcd(left: SqlDecimal | null, right: SqlDecimal | null): SqlDecimal | null { return sqlDecimalGcd(left, right, false); }
function decimalLcm(left: SqlDecimal | null, right: SqlDecimal | null): SqlDecimal | null { return sqlDecimalGcd(left, right, true); }
function decimalFromInteger(value: bigint | null): SqlDecimal | null { return decimalInput(value === null ? null : value.toString()); }
class SqlDecimalIntegerSpecialError extends Error {
    readonly code = "0A000";
    constructor() { super("cannot convert special numeric value to integer"); }
}
function int2FromDecimal(value: SqlDecimal | null): bigint | null {
    if (value === null)
        return null;
    if (!value.value.isFinite())
        throw new SqlDecimalIntegerSpecialError();
    const rounded = sqlDecimalRound(value, 0n, false)!.value;
    if (rounded.lt("-32768") || rounded.gt("32767"))
        throw new SqlIntegerError();
    return sqlIntegerRange(BigInt(rounded.toFixed(0)), -32768n, 32767n);
}
function int4FromDecimal(value: SqlDecimal | null): bigint | null {
    if (value === null)
        return null;
    if (!value.value.isFinite())
        throw new SqlDecimalIntegerSpecialError();
    const rounded = sqlDecimalRound(value, 0n, false)!.value;
    if (rounded.lt("-2147483648") || rounded.gt("2147483647"))
        throw new SqlIntegerError();
    return sqlIntegerRange(BigInt(rounded.toFixed(0)), -2147483648n, 2147483647n);
}
function int8FromDecimal(value: SqlDecimal | null): bigint | null {
    if (value === null)
        return null;
    if (!value.value.isFinite())
        throw new SqlDecimalIntegerSpecialError();
    const rounded = sqlDecimalRound(value, 0n, false)!.value;
    if (rounded.lt("-9223372036854775808") || rounded.gt("9223372036854775807"))
        throw new SqlIntegerError();
    return sqlIntegerRange(BigInt(rounded.toFixed(0)), -9223372036854775808n, 9223372036854775807n);
}
function sqlDecimalExactFloat(value: number): Decimal {
...
function sqlDecimalFromFloat(value: number | null, digits: number): SqlDecimal | null {
    return value === null ? null : decimalInput(sqlDecimalExactFloat(value).toSD(digits, Decimal.ROUND_HALF_EVEN).toString());
}
function decimalFromFloat4(value: number | null): SqlDecimal | null { return sqlDecimalFromFloat(value, 6); }
function float4FromDecimal(value: SqlDecimal | null): number | null {
    if (value === null)
        return null;
    if (!value.value.isFinite())
    ...
    if (magnitude.gte(maximum) || magnitude.lte(new Constructor(2).pow(-150)))
        throw new SqlIntegerError();
    ...
    const upper = bits === 2139095039 ? maximum : center.add(at(bits + 1)).div(2);
    if (magnitude.lt(lower) || (magnitude.eq(lower) && (bits & 1) !== 0))
        bits--;
    else if (magnitude.gt(upper) || (magnitude.eq(upper) && (bits & 1) !== 0))
        bits++;
    view.setUint32(0, bits);
    const result = view.getFloat32(0);
    return value.value.isNegative() ? -result : result;
}
function decimalFromFloat8(value: number | null): SqlDecimal | null { return sqlDecimalFromFloat(value, 15); }
function float8FromDecimal(value: SqlDecimal | null): number | null {
    if (value === null)
        return null;
    const result = Number(value.value.toString());
    if (value.value.isFinite() && (!Number.isFinite(result) || (result === 0 && !value.value.isZero())))
        throw new SqlIntegerError();
    return result;
}
function decimalTypmod(value: SqlDecimal | null, typmod: bigint | null): SqlDecimal | null {
    if (value === null || typmod === null)
        return null;
    if (typmod < 4n || value.value.isNaN())
        return value;
    if (!value.value.isFinite())
        throw new SqlIntegerError();
