...
func (q *Queries) ReturnMany(ctx context.Context, params ReturnManyParams) ([]ReturnManyRow, error) {
	arrayInput1, err := pgsidpgx.ValidateArrayInput[int32](params.Flexible, []int{1, 2}, "ReturnMany", "flexible", false)
	if err != nil {
		return nil, err
	}
	...
	for rows.Next() {
		...
		if err := rows.Scan(pgsidpgx.ArrayTarget[int32](&row.Flexible, []int{1, 2}, "ReturnMany", "flexible", false)); err != nil {
			return nil, err
		}
