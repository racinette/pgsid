...
func (q *Queries) GetView(ctx context.Context) (GetViewRow, error) {
	var row GetViewRow
	err := q.db.QueryRow(ctx, GetViewSQL).Scan(pgsidpgx.ScanTargets(pgsidpgx.ArrayTarget[pgsid.Null[int32]](&row.Matrix, []int{2}, "GetView", "matrix", false), pgsidpgx.ArrayTarget[pgsid.Null[int32]](&row.Flexible, []int{1, 2}, "GetView", "flexible", false)))
	return row, err
}
