...
func (q *Queries) ReturningPayload(ctx context.Context, params ReturningPayloadParams) (ReturningPayloadRow, error) {
	...
	err = q.db.QueryRow(ctx, ReturningPayloadSQL, jsonInput1).Scan(pgsidpgx.ValidatedJSON(&row.Payload, "pgsid:///jsonschemas/Event.json#", "ReturningPayload", "payload", false))
	return row, err
}
