...
func (q *Queries) UpdateDocument(ctx context.Context, params UpdateDocumentParams) (UpdateDocumentRow, error) {
	...
	err := q.db.QueryRow(ctx, UpdateDocumentSQL, params.Payload).Scan(pgsidpgx.ValidatedJSON(&row.Payload, updateDocumentJSONValidation, "UpdateDocument", "payload", false))
	return row, err
}
