...
func (q *Queries) GetPrototype(ctx context.Context) (GetPrototypeRow, error) {
	...
	err := q.db.QueryRow(ctx, GetPrototypeSQL).Scan(pgsidpgx.ScanTargets(pgsidpgx.ValidatedJSON(&row.Proto, "pgsid:///jsonschemas/Document.json#", "GetPrototype", "__proto__", false)))
	return row, err
}
