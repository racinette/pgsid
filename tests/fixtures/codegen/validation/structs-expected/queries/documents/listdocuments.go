...
func (q *Queries) ListDocuments(ctx context.Context) ([]ListDocumentsRow, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(pgsidpgx.ScanTargets(pgsidpgx.ValidatedJSON(&row.Payload, listDocumentsJSONValidation, "ListDocuments", "payload", false))); err != nil {
			return nil, err
		}
