...
func (q *Queries) GetDocument(ctx context.Context) (GetDocumentRow, error) {
	...
	err := q.db.QueryRow(ctx, GetDocumentSQL).Scan(pgsidpgx.ScanTargets(pgsidpgx.ValidatedJSON(&row.Payload, getDocumentJSONValidation, "GetDocument", "payload", false), pgsidpgx.ValidatedJSON(&row.Node, getDocumentJSONValidation, "GetDocument", "node", true), &row.Unchecked, pgsidpgx.ValidatedJSON(&row.Forced, getDocumentJSONValidation, "GetDocument", "forced", false), pgsidpgx.ValidatedJSON(&row.Maybe, getDocumentJSONValidation, "GetDocument", "maybe", true), pgsidpgx.ValidatedJSON(&row.Strict, getDocumentJSONValidation, "GetDocument", "strict", true)))
	return row, err
}
