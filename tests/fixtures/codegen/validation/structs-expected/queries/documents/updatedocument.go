...
func (q *Queries) UpdateDocument(ctx context.Context, params UpdateDocumentParams) (UpdateDocumentRow, error) {
	...
	err = q.db.QueryRow(ctx, UpdateDocumentSQL, jsonInput1).Scan(pgsidpgx.ScanTargets(pgsidpgx.ValidatedJSON(&row.Payload, "pgsid:///jsonschemas/Document.json#", "UpdateDocument", "payload", false)))
	return row, err
}
