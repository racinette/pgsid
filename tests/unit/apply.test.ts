...
describe('apply: migration pipeline', () => {
  ...
  async function applyAndValidate(sql: string) {
    ...
    const validateDiags: SqlDiagnostic[] = result.success ? await builder.validate(pg) : []
    try {
      await pg.exec('DROP SCHEMA IF EXISTS test_apply CASCADE;')
    } catch {
      /* ignore */
    }
  ...
  describe('DDL/DML failures', () => {
    ...
    it('reports an undefined-column error with position (PGlite exec)', async () => {
      ...
      const source = Buffer.from(sql, 'utf8')
      const textAtRange = source
        .subarray(result.diagnostics[0]!.range!.start, result.diagnostics[0]!.range!.end)
        .toString('utf8')
