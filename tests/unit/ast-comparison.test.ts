...
$$;`,
  },
  {
    ...
    expectByteSearch: true,
    createSql:
      'CREATE FUNCTION public.fmt_tabs(\n\t\ta int,\n\t\tb text\n\t) RETURNS int LANGUAGE plpgsql AS $$\nBEGIN\n  RETURN a + 1;\nEND;\n$$;',
  },
  {
...
$outer$;`,
  },
  {
    ...
    expectByteSearch: true,
    createSql:
      'CREATE FUNCTION public.body_weird_ws(a int) RETURNS int LANGUAGE plpgsql AS $$\n\t\tBEGIN\n\t\t\t\tRETURN\t\ta\t+\t1;\n\t\tEND;\n\t$$;',
  },
  {
    id: 'body_comments',
...
describe('AST comparison: introspection vs migration', () => {
  ...
  describe('overloads: same name, different arg types', () => {
    beforeAll(async () => {
      await pg.exec(
        'CREATE FUNCTION public.ovl(x int) RETURNS int LANGUAGE sql AS $$ SELECT x + 1; $$;',
      )
      await pg.exec(
        "CREATE FUNCTION public.ovl(x text) RETURNS text LANGUAGE sql AS $$ SELECT x || '!'; $$;",
      )
    })
