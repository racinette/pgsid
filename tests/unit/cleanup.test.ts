...
describe('cleanupPg', () => {
  ...
  it('after creating objects + cleanup → all gone', async () => {
    ...
    await pg.exec(
      'CREATE FUNCTION public.f1() RETURNS void LANGUAGE plpgsql AS $$ BEGIN PERFORM 1; END; $$;',
    )
    await pg.exec('CREATE SCHEMA s1;')
    await pg.exec('CREATE TABLE s1.t2 (id int);')
    await pg.exec(
      'CREATE FUNCTION s1.f2() RETURNS void LANGUAGE plpgsql AS $$ BEGIN PERFORM 1; END; $$;',
    )
