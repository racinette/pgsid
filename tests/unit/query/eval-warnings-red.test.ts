...
const CLOSED_GUARD = 'SELECT CASE WHEN 2 + 2 = 4 THEN o.id ELSE NULL END AS c FROM orders o'

/** A closed set-returning call, so the cardinality round has a question. */
const CLOSED_SRF = "SELECT * FROM ROWS FROM (dom_lenient('a'), generate_series(1, 1))"
