...
describe('parameter NULL-rejection mechanisms (PostgreSQL behaviour)', () => {
  ...
  const bindTime: [label: string, sql: string, params: unknown[]][] = [
    ...
    ['cast under a false CASE guard', "SELECT CASE WHEN false THEN $1::uname ELSE 'x' END", [null]],
    [
  ...
  it('rejects writes to GENERATED ALWAYS columns before execution', async () => {
    ...
    `)
    expect(
      await errorOf('INSERT INTO gen_t (a, gen, b) VALUES ($1, $2, $3)', [1, 2, 'x']),
  ...
  it("bp control: substitution must cast to the COLUMN's type, or it answers a different question", async () => {
    ...
    const asBp = await pg.query<{ g: boolean }>("SELECT ('a'::char(4) = 'a '::char(4)) AS g")
  ...
  it("a STABLE body's analysis-time answer does not bind enforcement — evaluate immutable only", async () => {
    ...
    expect(await errorOf('INSERT INTO lim VALUES (5)', [])).toContain('violates check constraint')
  ...
  it('a stable INPUT function makes even an immutable call session-dependent', async () => {
    ...
    const mdy = await pg.query<{ v: number }>("SELECT date_part('day', '1/2/2020'::date) AS v")
    await pg.exec("SET datestyle = 'ISO, DMY'")
    const dmy = await pg.query<{ v: number }>("SELECT date_part('day', '1/2/2020'::date) AS v")
  ...
  it("hash bounds render as a satisfies_hash_partition call over the parent's OID", async () => {
    ...
    const oid = (await pg.query<{ o: string }>("SELECT 'pb_h'::regclass::oid::text AS o")).rows[0]!
      .o
  ...
  it('NULL routing: range NULL keys go to DEFAULT; a non-default range partition never holds one', async () => {
    ...
    expect(routed.rows.map((r) => r.part)).toEqual([
      'pb_r1',
      'pb_r1',
      'pb_rmax',
      'pb_rmin',
  ...
  it('ATTACH validates every existing row against the bound; DETACH clears it', async () => {
    ...
    expect(await partDef('pb_att')).toBe('((id IS NOT NULL) AND (id >= 200) AND (id < 300))')
  ...
  it('every admitted shape parses to the SAME value under all 12 DateStyle settings', async () => {
    ...
    const invariant: [label: string, expr: string][] = [
      ...
      [
        'ts fraction',
        "'2020-01-01 12:34:56.789'::timestamp = make_timestamp(2020,1,1,12,34,56.789)",
      ],
      ...
      [
        'ts surrounding spaces',
        "' 2020-01-01 12:34:56 '::timestamp = make_timestamp(2020,1,1,12,34,56)",
      ],
      [
        'tstz surrounding spaces',
        "' 2020-01-01 12:34:56+00 '::timestamptz = make_timestamptz(2020,1,1,12,34,56,'UTC')",
      ],
      [
        'tstz offset hh:mm',
        "'2020-01-01 12:34:56+05:30'::timestamptz = make_timestamptz(2020,1,1,12,34,56,'+05:30')",
      ],
      [
        'tstz offset hh',
        "'2020-01-01 12:34:56+00'::timestamptz = make_timestamptz(2020,1,1,12,34,56,'UTC')",
      ...
      [
        'tstz T-sep offset',
        "'2020-01-01T12:34:56+00'::timestamptz = make_timestamptz(2020,1,1,12,34,56,'UTC')",
      ],
      ...
      [
        'non-padded tstz',
        "'2020-1-2 12:34:56+00'::timestamptz = make_timestamptz(2020,1,2,12,34,56,'UTC')",
      ],
  ...
  it('an offset-less timestamptz moves with TimeZone; an explicit offset pins the instant', async () => {
    ...
    const epoch = async (lit: string) => {
      const r = await pg.query<{ v: string }>(`SELECT extract(epoch from ${lit})::text AS v`)
      return r.rows[0]!.v
    }
  ...
  it("a VALUES body: no set-returning calls, equal row lengths, and COALESCE's unification", async () => {
    ...
    // operation's may not.
    const first = await pg.query<{ column1: number }>('SELECT * FROM (VALUES (2),(1)) q LIMIT 1')
  ...
  it("an ORDER BY orders the KEY's class, not the VALUE — which closes the sliced-sort question", async () => {
    ...
    const united = async (a: string, b: string): Promise<string> =>
      (
        await pg.query<{ v: string }>(
          `SELECT (SELECT ${a} UNION SELECT ${b} ORDER BY 1 LIMIT 1)::text AS v`,
        )
      ).rows[0]!.v
    expect(await united('1.00', '1.0')).toBe('1.00')
    expect(await united('1.0', '1.00')).toBe('1.0')
    expect(
      await errorOf('SELECT (SELECT 1.00 UNION SELECT 1.0 ORDER BY 1, 1::text LIMIT 1)', []),
    ).toContain('invalid UNION/INTERSECT/EXCEPT ORDER BY clause')
  })
  ...
  it('ON CONFLICT checks the proposed row BEFORE the arbiter — an insert keeps its universal footing', async () => {
    ...
    expect(
      await errorOf("INSERT INTO arc VALUES (1, 2, 'y') ON CONFLICT DO NOTHING", []),
    ).toContain('violates check constraint')
    expect(
      await errorOf(
        "INSERT INTO arc VALUES (1, 2, 'y') ON CONFLICT (id) DO UPDATE SET n = 'z'",
        [],
      ),
    ).toContain('violates check constraint')
    ...
    expect(
      await errorOf("INSERT INTO arc VALUES (1, 9, 'y') ON CONFLICT DO NOTHING", []),
    ).toBeNull()
