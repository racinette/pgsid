...
function joinChains(v: unknown, out: Counter, cur: string[] = []): void {
  if (Array.isArray(v)) {
    for (const x of v) joinChains(x, out, cur)
    return
  }
...
function recordStatement(stmt: unknown, s: Shape): void {
  ...
  if (root === 'InsertStmt') {
    ...
    if (sel && nonEmpty(sel[1].valuesLists))
      bump(
        s.dml,
        (sel[1].valuesLists as unknown[]).length > 1
  ...
  bump(
    s.cteCount,
    ctes.length === 0 ? '0' : ctes.length === 1 ? '1' : ctes.length <= 3 ? '2-3' : '4+',
  )
...
function report(s: Shape): void {
  ...
  console.log(
    `generated columns: ${s.generated}, of which ` +
