...
SELECT
  ...
  -- The three named-output columns above are still uniformly conservative:
  -- their shape comes from the snapshot one branch earlier, and no value
  -- reading reaches them. `g` takes the fallback branch, where the single
  -- column IS the call — builtin-from-position-value.sql holds that pair.
  g               -- @notNull
