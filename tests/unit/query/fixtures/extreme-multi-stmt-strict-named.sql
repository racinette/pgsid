...
SELECT
  multi_stmt_fn(p.name)             AS pos_from_table,    -- 
  multi_stmt_atomic(p.name, p.sku)  AS atomic_from_table, -- 
  strict_multi(p.name)              AS strict_pos_allnn,  -- 
  strict_multi(p.deleted_at::text)  AS strict_pos_nullarg, -- 
  strict_multi_atomic(p.name, p.sku) AS strict_atomic_allnn,  -- 
  strict_multi_atomic(p.name, p.deleted_at::text) AS strict_atomic_nullarg  -- 
FROM products p
