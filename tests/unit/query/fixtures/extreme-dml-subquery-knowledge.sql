...
SELECT
  insert_tag(p.name)                           AS insert_from_fn,  -- 
  COALESCE(insert_tag(p.name), 0)              AS safe_insert,    -- 
  (SELECT insert_tag(p.name))                   AS scalar_subquery,  -- 
  update_tag_price(p.id, p.name)              AS update_from_fn  -- 
FROM products p
