...
SELECT
  insert_tag(p.name)               AS insert_id,    -- 
  insert_tag(NULL)                 AS insert_nullarg, -- 
  update_tag_price(p.id, p.name)  AS update_result, -- 
  COALESCE(update_tag_price(1, p.name), 'none') AS safe_update  -- 
FROM products p
