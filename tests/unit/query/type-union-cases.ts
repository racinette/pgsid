...
export const UNION_CASES: Record<string, UnionCase[]> = {
  ...
  'arithmetic, same type': [
    {
      sql: 'SELECT abs(m.i + m.j) AS v FROM m',
      expect: { 'm.i + m.j': ['integer'], 'm.i': ['integer'] },
    },
  ...
  'arithmetic, mixed operands — the union is real': [
    {
      sql: 'SELECT abs(m.i + m.n) AS v FROM m',
      expect: { 'm.i + m.n': ['double precision', 'numeric', 'real'] },
    },
    {
      sql: 'SELECT abs(m.r + m.n) AS v FROM m',
      expect: { 'm.r + m.n': ['double precision', 'real'] },
    },
    {
      sql: 'SELECT abs(m.n * 2) AS v FROM m',
      expect: { 'm.n * 2': ['double precision', 'numeric', 'real'] },
    },
  ],
  ...
  'domains and enums': [
    {
      sql: 'SELECT abs(dm.p + 1) AS v FROM dm',
      expect: {
        'dm.p': ['public.pos'],
        'dm.p + 1': ['bigint', 'double precision', 'integer', 'numeric', 'real'],
      },
    },
    {
      sql: 'SELECT length(dm.e::text) AS v FROM dm',
      expect: { 'dm.e': ['public.mood'], 'dm.e::text': ['text'] },
    },
  ],
  ...
  concatenation: [
    ...
    {
      sql: 'SELECT length(m.t || m.v) AS v FROM m',
      expect: { 'm.t || m.v': ['anycompatiblearray', 'text'] },
    },
    {
      sql: "SELECT length(m.t || 'x') AS v FROM m",
      expect: { "m.t || 'x'": ['anycompatiblearray', 'text'] },
    },
    {
      sql: 'SELECT array_length(m.arr || m.arr, 1) AS v FROM m',
      expect: { 'm.arr || m.arr': ['anycompatiblearray'] },
    },
  ...
  'function calls with overloads': [
    ...
    {
      sql: 'SELECT length(substr(m.t, 1, 3)) AS v FROM m',
      expect: { 'substr(m.t, 1, 3)': ['text'] },
    },
    {
      sql: "SELECT abs(date_part('day', m.ts)) AS v FROM m",
      expect: { "date_part('day', m.ts)": ['double precision'] },
    },
    {
      sql: "SELECT length(to_char(m.ts, 'YYYY')) AS v FROM m",
      expect: { "to_char(m.ts, 'YYYY')": ['text'] },
    },
  ],
  ...
  'nesting, depth 2-3': [
    ...
    },
    {
      ...
      expect: {
        'm.n * 2': ['double precision', 'numeric', 'real'],
        'round(m.n * 2, 1)': ['numeric'],
      },
    },
    {
  ...
  casts: [
    {
      sql: 'SELECT abs(CAST(m.i AS bigint)) AS v FROM m',
      expect: { 'CAST(m.i AS bigint)': ['int8'] },
    },
    // ... and the same spelling then propagates: `int4 + integer` still
    // resolves to one candidate, whose RETURN is the catalog's `integer`.
    {
      sql: 'SELECT abs(m.n::integer + 1) AS v FROM m',
      expect: { 'm.n::int': ['int4'], 'm.n::int + 1': ['integer'] },
    },
  ],
  ...
  'date and interval': [
    ...
    {
      sql: 'SELECT (m.ts + m.iv) < m.ts AS v FROM m',
      expect: { 'm.ts + m.iv': ['timestamp with time zone'] },
    },
    {
      sql: 'SELECT (m.d + m.iv) < m.ts AS v FROM m',
      expect: { 'm.d + m.iv': ['timestamp without time zone'] },
    },
  ],
  ...
  'member-list nodes': [
    {
      sql: 'SELECT abs((CASE WHEN m.i > 0 THEN m.i ELSE m.j END)) AS v FROM m',
      expect: { 'CASE WHEN m.i > 0 THEN m.i ELSE m.j END': ['integer'] },
    },
    {
      sql: 'SELECT abs((CASE WHEN m.i > 0 THEN m.i ELSE m.n END)) AS v FROM m',
      expect: { 'CASE WHEN m.i > 0 THEN m.i ELSE m.n END': ['integer', 'numeric'] },
    },
    // No ELSE: the branch types still decide, and the missing branch is a
    // NULLABILITY fact, not a typing one.
    {
      sql: 'SELECT abs((CASE WHEN m.i > 0 THEN m.i END)) AS v FROM m',
      expect: { 'CASE WHEN m.i > 0 THEN m.i END': ['integer'] },
    },
    {
      sql: 'SELECT abs(COALESCE(m.i, m.j)) AS v FROM m',
      expect: { 'COALESCE(m.i, m.j)': ['integer'] },
    },
    {
      sql: 'SELECT abs(COALESCE(m.i, m.n)) AS v FROM m',
      expect: { 'COALESCE(m.i, m.n)': ['integer', 'numeric'] },
    },
    // An unknown member beside a known one contributes nothing and costs
    // nothing — the known members still type the node.
    {
      sql: "SELECT length(COALESCE(m.t, 'x')) AS v FROM m",
      expect: { "COALESCE(m.t, 'x')": ['text'] },
    },
    // ... but ALL-unknown stays null. The guard.
    {
      sql: "SELECT length(COALESCE('a', 'b')) AS v FROM m",
      expect: { "COALESCE('a', 'b')": null },
    },
    {
      sql: 'SELECT abs(GREATEST(m.i, m.n)) AS v FROM m',
      expect: { 'GREATEST(m.i, m.n)': ['integer', 'numeric'] },
    },
    {
      sql: 'SELECT array_length(ARRAY[m.t, m.t], 1) AS v FROM m',
      expect: { 'ARRAY[m.t, m.t]': ['text[]'] },
    },
    {
      sql: 'SELECT array_length(ARRAY[m.i, m.n], 1) AS v FROM m',
      expect: { 'ARRAY[m.i, m.n]': ['integer[]', 'numeric[]'] },
    },
    {
      sql: 'SELECT array_length(ARRAY[m.arr, m.arr], 1) AS v FROM m',
      expect: { 'ARRAY[m.arr, m.arr]': ['text[]'] },
    },
    {
      sql: "SELECT array_length(ARRAY['a', 'b'], 1) AS v FROM m",
      expect: { "ARRAY['a', 'b']": null },
    },
  ],
  ...
  'row expressions': [
    {
      sql: 'SELECT row_to_json(ROW(m.i, m.t)) AS v FROM m',
      expect: { 'ROW(m.i, m.t)': ['record'] },
    },
  ],
  ...
  'still untyped': [
    {
      sql: 'SELECT abs((SELECT max(m2.i) FROM m m2)) AS v FROM m',
      expect: { '(SELECT max(m2.i) FROM m AS m2)': null },
    },
  ],
}
