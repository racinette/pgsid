...
async function claim(sql: string, column?: string): Promise<boolean> {
  ...
  const evaluate = async (q: string) => (await pg.query<Record<string, unknown>>(q)).rows[0]
...
describe('what is still unreadable, and why that is a different question', () => {
  ...
  it('window and aggregate calls read no type', async () => {
    expect(
      (await readings(`WITH s AS (${OPAQUE_INTS}) SELECT s.v + s.v AS c FROM s`)).get('s.v'),
    ).toBeNull()
    expect(
      (
        await readings(`WITH s AS (SELECT max(r.n) AS v FROM route r) SELECT s.v + s.v AS c FROM s`)
      ).get('s.v'),
    ).toBeNull()
  })
})
