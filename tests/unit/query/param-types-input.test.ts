...
async function tracedFor(
  ...
  const cols = await inferNullabilityTraced(parsed.stmts![0]!.stmt!, catalog, undefined, {
    paramTypes,
  })
...
describe('tier 0: parameter types as an input', () => {
  ...
  it('closes the recorded || strictness over-report where the operand types', async () => {
    ...
    await expect(pg.query("INSERT INTO ct (name) VALUES ('a' || $1)", [null])).rejects.toThrow(
      /not-null constraint/,
    )
  })
