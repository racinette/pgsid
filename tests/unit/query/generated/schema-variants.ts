...
export const BASE_SCHEMA_SQL = readFileSync(join(__dirname, '..', 'fixtures', 'schema.sql'), 'utf8')
...
const columnsOf = (schema: string, table: string): ColumnMap => byColumn()[schema]?.[table] ?? {}
...
const drawFrom =
  ...
  (rand, ctx) => {
    ...
    if (vs.length === 0)
      throw new Error(`${table}.${column} emitted nothing for a foreign key to draw from`)
    return rand.pick(vs)
  }
...
`,
  ...
  },
  {
    ...
    why: "`ConstraintType` admits `exclusion` and no fixture ever produced one, so the walk's constraint reading had never been shown to IGNORE it — an exclusion constraint is not a CHECK and must not be read as one. Placed on `t`, which every structure in the corpus scans, so the constraint is in the list the walk actually consults.",
...
`,
  ...
  },
  {
    ...
    why: '`v` becomes a MATERIALIZED VIEW over the real rows. The adapter folds matviews in beside views at three sites and analyses their definition instead of the catalog flag — PostgreSQL propagates no `attnotnull` to either — and nothing checked that the two behave alike. The corpus joins `v` in every deep structure, so the definition analysis is exercised across the whole space.',
...
`,
  ...
  {
    name: 'inherit-child',
    why: "`ALTER TABLE ONLY t … SET NOT NULL` leaves a child free to store the NULL the parent forbids, and `FROM t` scans the whole tree — so the parent's own flag is the wrong question and `notNullTree` is the right one. The corpus reads `t.name` everywhere, so a child row with a NULL name falsifies any claim that took the parent's flag.",
...
`,
  ...
  {
    name: 'second-schema',
    why: 'The corpus references `t`, `u` and `v` UNQUALIFIED, so which relation they name is a search-path question — the one sweep-3 found the engine had backwards. A shadow schema first in the path makes every structure resolve through `inPath`, with the public originals still present as the wrong answer.',
    covers: ['second-schema', 'relation-name-in-two-schemas', 'function-overloaded-across-schemas'],
