...
import { generateQueries, generateDeepJoinQueries, type GeneratedQuery } from './generator.js'
  ...
  for (const q of chosen as GeneratedQuery[]) {
    ...
    try {
      ...
      if (e instanceof UnsupportedNodeError) result.refused++
        ...
        result.crashes.push({ schema: variant.name, state: '-', sql, detail: (e as Error).message })
      continue
    }
  ...
  it('PostgreSQL accepts every generated query, under every schema', () => {
    ...
    expect(
      ...
      `A generated query was rejected. Under a VARIANT schema that is a ` +
        ...
        bad
          .slice(0, 10)
          .map((f) => `[${f.schema}/${f.state}] ${f.detail}\n    ${f.sql}`)
          .join('\n  '),
    ).toEqual([])
  })
