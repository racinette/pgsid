...
const expectParams = (...numbers: number[]): Expectation => ({
  ...
  present: (root) => {
    ...
    return found.size === numbers.length && numbers.every((n) => found.has(n))
  },
})
...
function joinStructures(): JoinStructure[] {
  ...
  for (const k of JOIN_KINDS) {
    out.push({
      ...
      fromClause: [join(k, onlyVar('t'), onlyVar('u'), eq(colRef('u', 't_id'), colRef('t', 'id')))],
      slots: tuSlots,
      extras: [],
    ...
    RangeSubselect: {
      ...
      subquery: {
        SelectStmt: bareSelect({
          targetList: [target(colRef('u', 'email'), 'lemail'), target(colRef('u', 'val'), 'lval')],
  ...
  const srfItem = (lateral: boolean): Ast => ({
    RangeFunction: {
      ...
      functions: [{ List: { items: [funcCall('gfn_urows', [colRef('t', 'id')]), {}] } }],
      alias: { aliasname: 'g' },
    },
  })
  ...
  const jsonEachItem = (): Ast => ({
    RangeFunction: {
      functions: [
        {
          List: {
            items: [funcCall('json_each_text', [castTo(textConst('{"a": null}'), 'json')]), {}],
          },
        },
      ],
      alias: { aliasname: 'je' },
...
const PROJECTIONS: Projection[] = [
  ...
  },
  {
    ...
    key: 'check-lit',
    build: (s) => ({
      targets: [target(colRef('t', 'name'), 'a_ckn'), target(s.slots.intKey, 'a_int')],
...
export function generateDmlQueries(): GeneratedQuery[] {
    ...
    for (const [key, targets] of updateReturning) {
      dml(
        ...
        key,
        {
          UpdateStmt: {
            relation: relation('v'),
            targetList: [setItem('amount', paramRef(1)), setItem('u_id', paramRef(2))],
            fromClause: [tJoinU(k)],
  ...
  for (const k of JOIN_KINDS) {
    ...
    for (const [key, targets] of deleteReturning) {
      dml(
        ...
        [{ number: 1, valid: 999 }],
        [expect('DELETE', 'DeleteStmt'), expectReturning, expectJoins(k), expectParams(1)],
      )
    }
  }
  ...
  // ever witnessed NULL.
  {
    ...
    const kinds: [
      ...
      ['merge-nothing', srcPlain, 'src-plain', [NOTHING, INS(1)], [{ number: 1, valid: 'mn' }]],
      [
        'merge-join-src',
        srcJoin,
        'src-join(inner)',
    ...
    for (const k1 of JOIN_KINDS) {
      for (const k2 of JOIN_KINDS) {
        for (const k3 of JOIN_KINDS) {
          const structure = `deep-${shape.key}(${kindLabel(k1)},${kindLabel(k2)},${kindLabel(k3)})`
          out.push({
