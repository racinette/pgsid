...
function taggedTypes(v: unknown, out: Set<string>): void {
  if (Array.isArray(v)) {
    for (const x of v) taggedTypes(x, out)
    return
  }
  ...
  beforeAll(async () => {
    ...
    const rows = [...borrowed.byType.keys()]
      .map((t) => ({
        t,
        ours: rate(ours, t),
        theirs: rate(borrowed, t),
        n: ours.byType.get(t) ?? 0,
      }))
    ...
    console.log(
      `  ${'node'.padEnd(22)}${'ours'.padStart(8)}${'borrowed'.padStart(10)}${'our stmts'.padStart(11)}`,
    )
    for (const r of rows.slice(0, 15))
      console.log(
        `  ${r.t.padEnd(22)}${r.ours.toFixed(2).padStart(8)}` +
    ...
    console.log(
      '  Read these as absent in TAGGED form — see the header before ' +
