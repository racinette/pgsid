 ...
 */
export const CONSTRAINT_REJECTION = /violates check constraint|violates partition constraint/
  ...
  for (const line of content.split('\n')) {
    ...
    if (plannerKeepsOpen) {
      ...
      if (cont) {
        ...
        plannerKeeps = {
          count: plannerKeeps!.count,
          reason: `${plannerKeeps!.reason} ${cont[1]!.trim()}`,
        }
        continue
      }
    ...
    if (searchPathMatch) {
      ...
      for (const schema of schemas) {
        if (!/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(schema)) {
          throw new Error(`@search-path schema names are bare identifiers; got: ${schema}`)
        }
      }
