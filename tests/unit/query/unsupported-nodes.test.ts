...
describe('unsupported nodes are refused, not guessed', () => {
  ...
  it('refuses (expr).* when the composite cannot be resolved', async () => {
    await expect(infer('SELECT (ROW(1, 2)::no_such_type).* FROM t')).rejects.toMatchObject({
      name: 'UnsupportedNodeError',
      site: 'composite-star',
    })
    await expect(infer('SELECT (s.c).* FROM (SELECT ROW(1, 2) AS c) s')).rejects.toMatchObject({
      name: 'UnsupportedNodeError',
      site: 'composite-star',
    })
  })
  ...
  it('does not refuse unnest whose element type the catalog answers', async () => {
    const cases: [string, string[]][] = [
      ...
      ["SELECT * FROM unnest(array_remove(ARRAY[ROW('a', 1)::sku_pair], NULL))", ['sku', 'qty']],
      [
  ...
  it('refuses an unknown FROM item', async () => {
    ...
    const stmt = {
      SelectStmt: {
        targetList: [{ ResTarget: { name: 'a', val: { A_Const: { ival: { ival: 1 } } } } }],
