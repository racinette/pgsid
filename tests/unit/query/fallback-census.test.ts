...
const hasFact = (o: Obs, name: string, value: string): boolean => o.facts.has(factKey(name, value))
const hasFactWhere = (o: Obs, name: string, pred: (value: string) => boolean): boolean =>
  o.factList.some((f) => f.name === name && pred(f.value))
...
const strictnessSomeDeclined = (calls: RecordedCall[], op: (name: string) => boolean): boolean =>
  calls.some(
    (c) =>
      c.member === 'resolveOperatorStrictnessSome' && c.result === null && op(String(c.args[1])),
  )
...
const INVENTORY: Record<string, FallbackEntry> = {
  ...
  ...Object.fromEntries(
    Object.keys(PARTIAL_OVERLOADS).map((op) => [
      `aexpr-totality:partial-overload:${op}`,
      {
        ...
        direction: 'refuses',
        detect: (o: Obs) => hasFactWhere(o, 'partialOverload', (v) => v === PARTIAL_OVERLOADS[op]),
  ...
  'aexpr-totality:custom-operator-dispatch': {
    ...
    direction: 'claims',
    detect: (o) => hasFactWhere(o, 'customOperator', (v) => !v.endsWith('(type-narrowed)')),
  },
  ...
  'forced-null-by:strict-operator-name': {
    ...
    direction: 'claims',
    detect: (o) =>
      strictnessSomeDeclined([...o.walkCalls, ...o.contractCalls], (n) => STRICT_OPERATORS.has(n)),
  },
  ...
  'forced-null-by:builtin-name': {
    ...
    detect: (o) =>
      ...
      o.contractCalls.some(
        (c) =>
          c.member === 'isStrictBuiltin' && metadataDeclined(o.contractCalls, String(c.args[0])),
      ),
  },
}
...
const WITNESSES: Record<string, Witness[]> = {
  ...
  'promotion-strictness:non-strict-overload:||': [{ file: 'non-strict-overload-promotion.sql' }],
  'aexpr-totality:name-claim': [
...
describe('fallback census', () => {
  ...
  it('every reached entry is pinned by witness NAME', () => {
    ...
    const unpinned = [...reached.keys()].filter((key) => !WITNESSES[key]?.length).sort()
    expect(
      unpinned,
    ...
    for (const [key, witnesses] of Object.entries(WITNESSES)) {
      ...
      for (const w of witnesses) {
        ...
        if (w.check && !w.check(obs)) {
          failures.push(
            `${key} — ${w.file} reaches the signal but the site-pinning outcome ` + `check failed`,
          )
        }
      }
    }
