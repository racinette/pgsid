...
describe('rung census (decision-site reach)', () => {
  ...
  afterAll(() => {
    ...
    if (process.env.RUNG_CENSUS_REPORT) {
      ...
      console.log(
        `\ndark rungs:\n` +
          cold.map((p) => `  ${JSON.stringify(p.key)} (line ${p.lines.join(', ')})`).join('\n'),
      )
      if (unmatched.size) {
        console.log(
          `\nunmatched reasons:\n` +
  ...
  it('the floor holds: every declared rung fires with its declared outcomes, and nothing fires undeclared', () => {
    ...
    for (const [key, outcomes] of observed) {
      observedFlat[key] = outcomes.size === 2 ? 'both' : ([...outcomes][0] as Outcome)
    }
...
const FLOOR: Record<string, Outcome> = {
  ...
  "'⟨*⟩' carries a recorded non-total signature and the operand types did not narrow it away → nullable":
    'false',
  ...
  'WHEN[⟨*⟩] guard evaluated TRUE → later arms and ELSE never run; every reachable branch non-null → CASE non-null':
    'true',
  ...
  "operator '⟨*⟩' keeps a non-total or unvouched candidate for these operand types → nullable":
    'false',
  "operator '⟨*⟩' keeps a non-total or unvouched candidate for this operand type → nullable":
    'false',
...
const DARK_RUNGS: Record<string, string> = {
  ...
  "function param '⟨*⟩' → notNull":
    'INSTRUMENT-BLIND — named-param bodies (pass_two) reach it untraced.',
  ...
  "column '⟨*⟩' not found in the function's return type":
    'DEFENSIVE — PostgreSQL rejects the name.',
