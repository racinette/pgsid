...
async function verdict(sql: string): Promise<'notNull' | 'alwaysNull' | 'nullable'> {
  ...
  const evaluate = async (s: string) => (await pg.query<Record<string, unknown>>(s)).rows[0]
...
describe('transitive nullability through a generated column', () => {
  it("WHERE + CHECK pin the selected arm's operands", async () => {
    ...
    expect(await verdict('SELECT finished_at FROM ev WHERE status = 3 AND has_duration')).toBe(
      'notNull',
    )
  })
  ...
  it('the CHECK-less variant rides WHERE promotion alone', async () => {
    ...
    expect(
      await verdict('SELECT finished_at FROM ev WHERE status = 3 AND event_duration IS NOT NULL'),
    ).toBe('notNull')
  })
  ...
  it('a refuted guard concludes the generated column alwaysNull', async () => {
    ...
    expect(await verdict('SELECT finished_at FROM ev WHERE status = 1 AND has_duration')).toBe(
      'alwaysNull',
    )
  })
