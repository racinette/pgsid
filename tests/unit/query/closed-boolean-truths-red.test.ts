...
describe('closed boolean subexpressions in a statement predicate', () => {
  ...
  const WINS = [
    ...
    ['a dead arm among three', "SELECT v FROM plain_rows WHERE 1 > 2 OR v IS NOT NULL OR v = 'a'"],
    [
      'a dead arm in an ON clause',
      'SELECT p.v FROM plain_rows p JOIN plain_rows q ON q.id = p.id AND (1 > 2 OR p.v IS NOT NULL)',
    ],
  ] as const
  ...
  const BOUNDS = [
    ...
    ['an unprovable open disjunct', 'SELECT v FROM plain_rows WHERE v IS NOT NULL OR id > 0'],
  ] as const
