...
const SETTLED_ELSEWHERE: Record<string, string> = {
  ...
  'nth_value(anyelement,integer)':
    'witnessed in tests/unit/functions; a short frame has no Nth row',
...
const NO_GENERATOR: Record<string, string> = {
  ...
  unknown:
    'the type of an unadorned literal before resolution; it never survives to be an argument',
...
const UNPROBED: Record<string, readonly string[]> = {
  ...
  'collation-context': ['btvarstrequalimage(oid)'],
  ...
  wasm: ['pg_get_shmem_allocations_numa()', 'to_ascii(text)'],
}
...
describe('builtin scalar surface, witnessed or classified', () => {
  ...
  beforeAll(async () => {
    ...
    for (const r of rows) {
      ...
      const probeTypes = r.variadic === null ? r.types : [...r.types.slice(0, -1), r.variadic]
      ...
      if (r.aggkind === 'h') {
        ...
        if (r.types.length === 0) {
          ...
          const nulls = r.types
            ...
            .join(', ')
          tag([`(SELECT ${qualify(r.name)}(${nulls}) FROM (VALUES (1),(2)) s(z))`], 'all-null')
        }
      }
      if (mine.length === 0) {
        category.set(key, 'no-generator')
  ...
  it('classifies every scalar and operator signature into exactly one category', () => {
    ...
    if (snapshotPath) {
      ...
      for (const cat of ['null-witnessed', 'no-null-found', 'raised-everywhere', 'no-generator']) {
  ...
  it('every refused signature is either probed by a bounded call or recorded unprobed', () => {
    ...
    const unaccounted = Object.keys(REFUSED_CALLS)
      .filter(
        (k) => !(COHERENT_CALLS[k] ?? []).length && !OUT_OF_BAND_KEYS.has(k) && !recorded.has(k),
      )
      .sort()
    expect(
      unaccounted,
  ...
  it('the settled-elsewhere set is exactly what is recorded, in both directions', () => {
    ...
    const stale = Object.keys(SETTLED_ELSEWHERE)
  ...
  it('every type with no generator has a recorded reason, in both directions', () => {
    ...
    const stale = Object.keys(NO_GENERATOR)
      .filter((t) => !blocking.has(t))
      .sort()
    expect(
      unexplained,
