...
const CLASSIFICATION: Record<string, Classification> = {
  ...
  RangeTableSample: {
    category: 'handled',
    why: "TABLESAMPLE wraps its relation, and MARKS it sampled — the alias no longer stands for the whole table, so it is never a key's side and never preserved (sweep-4 finding 3)",
  },
  ...
  JsonTable: {
    category: 'handled',
    why: 'JSON_TABLE contributes COLUMNS; a NESTED PATH is an outer join against the level above it, so FOR ORDINALITY inside one is nullable (sweep-4 finding 5)',
  },
  RangeTableFuncCol: {
    category: 'structural',
    why: 'one XMLTABLE column, read via RangeTableFunc',
  },
  ...
  JsonTablePathSpec: {
    category: 'structural',
    why: 'path attached to a JSON_TABLE column',
    inlined: true,
  },
  ...
  Boolean: {
    category: 'structural',
    why: 'boolean leaf, inlined into A_Const as boolval',
    inlined: true,
  },
  BitString: {
    category: 'structural',
    why: 'bit-string leaf, inlined into A_Const as bsval',
    inlined: true,
  },
  ...
  CTECycleClause: {
    category: 'structural',
    why: 'generates cycle-mark and path columns',
    inlined: true,
  },
  ...
  JsonAggConstructor: {
    category: 'structural',
    why: 'shared aggregate scaffolding',
    inlined: true,
  },
  InferClause: { category: 'structural', why: 'ON CONFLICT inference target', inlined: true },
  OnConflictClause: {
    category: 'structural',
    why: 'ON CONFLICT action; consulted for row-count',
    inlined: true,
  },
