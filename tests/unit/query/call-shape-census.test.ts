...
const CENSUS: Record<string, ColumnVerdict> = {
  ...
  'pg_proc.pronamespace': {
    category: 'scope',
    why: 'pg_catalog only; a user function is not this surface',
  },
  'pg_proc.prokind': {
    category: 'shape',
    why: "'f' calls plainly, 'a' needs an aggregate context, 'w' needs OVER",
  },
  'pg_proc.proretset': {
    category: 'shape',
  ...
  'pg_proc.proargtypes': {
    category: 'shape',
    why: 'the declared input list a call lines up against',
  },
  'pg_proc.provariadic': {
    category: 'shape',
  ...
  'pg_proc.provolatile': {
    category: 'scope',
    why: "'v' is excluded on the catalog's own side-effect marker",
  },
  'pg_proc.proisstrict': {
    category: 'irrelevant',
  ...
  'pg_proc.proargnames': {
    category: 'irrelevant',
    why: 'named notation is a call SPELLING the probe never uses (and the walk skips typed dispatch for it)',
  },
  'pg_proc.proargdefaults': {
    category: 'irrelevant',
    why: 'the default EXPRESSIONS; the probe supplies every argument, so it never elides one',
  },
  'pg_proc.prosrc': {
    category: 'irrelevant',
    why: 'the C symbol or SQL body; totality lives in the implementation and a source scanner for it was built and discarded',
  },
  'pg_proc.prosqlbody': {
    category: 'irrelevant',
    why: 'the parsed body of a BEGIN ATOMIC function; no pg_catalog builtin has one',
  },
  'pg_proc.prolang': {
    category: 'irrelevant',
    why: 'every pg_catalog builtin of interest is internal or C; the language changes nothing about how a call is written',
  },
  'pg_proc.prosupport': {
    category: 'irrelevant',
    why: 'the planner support function, which never runs as part of evaluating the call',
  },
  'pg_proc.protrftypes': {
    category: 'irrelevant',
    why: 'transform types for PL languages; no bearing on a SQL-level call',
  },
  'pg_proc.probin': { category: 'irrelevant', why: 'the shared library path' },
  'pg_proc.proconfig': {
    category: 'irrelevant',
    why: 'per-function GUC settings; none is set on a pg_catalog builtin',
  },
  'pg_proc.proacl': { category: 'irrelevant', why: 'privileges; the probe runs as the owner' },
  'pg_proc.proowner': {
    category: 'irrelevant',
    why: 'ownership; the probe runs as the owner of everything in pg_catalog',
  },
  'pg_proc.procost': {
    category: 'irrelevant',
    why: 'planner cost estimate; it steers plan choice and never the value returned',
  },
  'pg_proc.prorows': {
    category: 'irrelevant',
    why: 'planner row estimate for a set function; an estimate, not a shape',
  },
  'pg_proc.prosecdef': {
    category: 'irrelevant',
    why: 'SECURITY DEFINER; changes who runs it, not what it returns',
  },
  'pg_proc.proleakproof': {
    category: 'irrelevant',
    why: 'whether it may leak values through errors; a security property',
  },
  'pg_proc.proparallel': {
    category: 'irrelevant',
    why: 'parallel safety, which decides where PostgreSQL may run it and not what it answers',
  },
  'pg_proc.pronargs': {
    category: 'irrelevant',
    why: 'the length of proargtypes, which the probe reads directly',
  },

  // --- pg_operator: the same questions for the infix and prefix forms -----
  'pg_operator.oid': {
    category: 'scope',
    why: 'identity; the join key oprcode and the amproc role predicate use',
  },
  'pg_operator.oprname': { category: 'shape', why: 'the symbol in the call' },
  'pg_operator.oprnamespace': {
    category: 'scope',
    why: "pg_catalog only; a user operator belongs to the walk's merged candidate set, not to this surface",
  },
  'pg_operator.oprkind': {
    category: 'irrelevant',
  ...
  'pg_operator.oprright': {
    category: 'shape',
    why: 'the right operand type, and the only operand a prefix operator has',
  },
  'pg_operator.oprcode': {
    category: 'shape',
  ...
  'pg_operator.oprresult': {
    category: 'irrelevant',
    why: 'the result type; the probe asks whether the value is NULL, and no operator returns a composite',
  },
  ...
  'pg_operator.oprcanmerge': {
    category: 'irrelevant',
    why: 'mergejoinability, a planner capability that changes no result the probe reads',
  },
  'pg_operator.oprcanhash': {
    category: 'irrelevant',
    why: 'hashjoinability, a planner capability that changes no result the probe reads',
  },
  ...
  'pg_cast.oid': {
    category: 'scope',
    why: 'identity; a cast row is addressed by its (source, target) pair, not by this',
  },
  ...
  'pg_aggregate.aggminitval': {
    category: 'irrelevant',
    why: 'the moving-aggregate INITCOND, for the same reason',
  },
  ...
  'pg_aggregate.aggsortop': {
    category: 'irrelevant',
    why: 'the sort operator MIN/MAX optimise through; a planner equivalence',
  },
  'pg_aggregate.aggtranstype': {
    category: 'irrelevant',
    why: 'the state type, which never appears in a call',
  },
  ...
  'pg_aggregate.aggfinalextra': {
    category: 'irrelevant',
    why: 'whether the final function gets extra dummy arguments; internal to execution',
  },
  'pg_aggregate.aggmfinalextra': { category: 'irrelevant', why: 'the moving-aggregate equivalent' },
  'pg_aggregate.aggfinalmodify': {
    category: 'irrelevant',
    why: 'whether the final function may modify state; internal to execution',
  },
  'pg_aggregate.aggmfinalmodify': {
    category: 'irrelevant',
    why: 'the moving-aggregate equivalent',
  },
}
...
describe('call-shape census: the catalog columns a probe must consult', () => {
  ...
  it('every catalog column is classified, and every classification is a real column', () => {
    ...
    const phantom = Object.keys(CENSUS)
