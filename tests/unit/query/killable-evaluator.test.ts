...
describe('killable evaluator', () => {
  it('kills a probe PGlite would never finish', async () => {
  ...
  it('survives the kill and answers the next probe', async () => {
    const ev = await make(500)
  ...
  it('carries the schema into the rebuilt instance', async () => {
  ...
  it('holds a search_path, and restores it after a kill', async () => {
    const ev = await createKillableEvaluator({
  ...
  it('holds a per-case scope, and re-opens it after a kill', async () => {
    const ev = await make(500)
