...
describe('totality tables, probed by execution', () => {
  ...
  beforeAll(async () => {
    ...
    const tableOf = (n: string): Signature['table'] =>
      ALWAYS_NOT_NULL_BUILTINS.has(n)
        ? 'alwaysNotNull'
        : FIRST_ARG_BUILTINS.has(n)
          ? 'firstArg'
          : 'strictTotal'
    ...
    const additionNames = new Set(
      [...STRICT_TOTAL_BUILTIN_SIGNATURES, ...SWEPT_TOTAL_SIGNATURES].map((k) =>
        k.slice(0, k.indexOf('(')),
      ),
    )
    ...
    for (const c of POSITIVE_CONTROL)
      if (verdicts.get(c) !== 'NULL') controlSurvivors.push(`${c} → ${verdicts.get(c)}`)
      ...
      if (
        !signatures.some(
          (s) =>
            s.table === 'operator' &&
            key ===
              `${s.name}(${s.prefix ? '' : s.types[0]},${s.prefix ? s.types[0] : s.types[1]})`,
        )
      ) {
  ...
  it('every STRICT_OPERATORS member is STRICT per pg_catalog', () => {
    ...
    const unrecorded = nonStrictOperators.filter(
      (sig) => !(sig.split(' ')[0]! in NON_STRICT_OVERLOADS),
    )
