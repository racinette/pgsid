...
describe('curated name tables vs pg_catalog', () => {
  ...
  it('every aggregate table holds only aggregates', () => {
    const wrong = [
      ...[...NON_NULL_OVER_NONEMPTY_AGGREGATES].map(
        (n) => ['NON_NULL_OVER_NONEMPTY_AGGREGATES', n] as const,
      ),
    ]
  ...
  it("the capture's aggkind classes match the catalog predicate", () => {
    ...
    const drift = (['h', 'o'] as const).flatMap((k) => {
      ...
      return [
        ...[...catalog]
          .filter((n) => !cap.has(n))
          .map((n) => `aggkind '${k}' capture is MISSING ${n}`),
        ...[...cap]
          .filter((n) => !catalog.has(n))
          .map((n) => `aggkind '${k}' capture has EXTRA ${n}`),
      ].sort()
    })
