...
describe('nullability-walk-traced', () => {
  ...
  it('traces WHERE promotion', async () => {
    ...
    expect(emailTrace.facts.some((f) => f.name === 'whereGuarantee' && f.value === 'true')).toBe(
      true,
    )
  })
  ...
  it('traces NOT NULL domain function return', async () => {
    ...
    expect(trace.decision).toBe(true)
    expect(
      trace.facts.some((f) => f.name === 'priority' && f.value === '1 (NOT NULL domain return)'),
    ).toBe(true)
  ...
  it('traces scalar subquery with count(*)', async () => {
    ...
    const trace = results[0]!.trace!
    expect(trace.decision).toBe(true)
    expect(trace.facts.some((f) => f.name === 'subLinkType' && f.value === 'EXPR_SUBLINK')).toBe(
      true,
    )
