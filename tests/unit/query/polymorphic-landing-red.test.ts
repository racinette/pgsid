...
describe('a polymorphic landing — the premise', () => {
  ...
  it('the closure face folds or refuses by whether an argument was SPELLED', () => {
    ...
    expect(face.closedFunctionTypes('array_position', [['text[]'], ['text']])).toEqual(['integer'])
    ...
    expect(face.closedFunctionTypes('array_append', [['text[]'], ['unknown']])).toEqual(['text[]'])
  })
})
...
describe('a polymorphic landing — the leak the check is for', () => {
  ...
  it('the face refuses it at the type level too', () => {
    ...
    expect(face.closedFunctionTypes('array_position', [['date[]'], ['date']])).toEqual(['integer'])
  })
})
