...
const anyColumn = (
  s: CatalogSnapshot,
  p: (c: CatalogSnapshot['tables'][number]['columns'][number]) => boolean,
): boolean => s.tables.some((t) => t.columns.some(p))

const anyConstraint = (
  s: CatalogSnapshot,
  p: (
    c: CatalogSnapshot['tables'][number]['constraints'][number],
    t: CatalogSnapshot['tables'][number],
  ) => boolean,
...
export const FEATURES: Record<string, Feature> = {
  ...
  'domain-over-scalar': {
    ...
    detect: (s) =>
      s.domains.some((d) => {
        ...
        return (
          !d.baseTypeName.endsWith('[]') &&
          !composites.has(d.baseTypeName) &&
          !domains.has(d.baseTypeName)
        )
      }),
  },
  'domain-not-null': {
  ...
  'second-schema': {
    ...
    detect: (s) =>
      s.schemas.some(
        (x) => x.name !== 'public' && x.name !== 'information_schema' && !x.name.startsWith('pg_'),
      ),
  },
  ...
  'quoted-identifier-with-embedded-quote': {
    ...
    detect: (s) =>
      s.functions.some((f) => quotedIdentifiers(f.returnType).some((id) => id.includes('""'))),
  },
  ...
  'setof-table-return': {
    ...
    detect: (s) => {
      const tables = qualified(s.tables)
      return s.functions.some(
        (f) => f.returnType.startsWith('SETOF ') && tables.has(f.returnType.slice(6)),
      )
    },
  },
  'setof-composite-return': {
    ...
    detect: (s) => {
      const composites = qualified(s.compositeTypes)
      return s.functions.some(
        (f) => f.returnType.startsWith('SETOF ') && composites.has(f.returnType.slice(6)),
      )
    },
  },
  'setof-domain-return': {
    ...
    detect: (s) => {
      const domains = qualified(s.domains)
      return s.functions.some(
        (f) => f.returnType.startsWith('SETOF ') && domains.has(f.returnType.slice(6)),
      )
    },
  },
  'setof-record-return-from-out-params': {
    ...
    detect: (s) =>
      s.functions.some(
        (f) => f.returnType === 'SETOF record' && f.args.some((a) => a.mode === 'out'),
      ),
  },
  'table-return': {
    category: 'handled',
  ...
  'composite-return-not-set': {
    ...
    detect: (s) => {
      ...
      const tables = qualified(s.tables)
      return s.functions.some(
        (f) => !f.returnsSet && (composites.has(f.returnType) || tables.has(f.returnType)),
      )
    },
  },
  'scalar-return': {
  ...
  'partition-leaf-carrying-a-trigger': {
    ...
    detect: (s) =>
      s.tables.some(
        (t) => t.writeRewrites.beforeRow.length === 0 && t.writeRewritesTree.beforeRow.length > 0,
      ),
  },
  'sub-partition': {
    category: 'handled',
  ...
  'do-instead-rule': {
    ...
    detect: (s) =>
      s.tables.some((t) => t.writeRewrites.insteadRules.length > 0) ||
      s.views.some((v) => v.writeRewrites.insteadRules.length > 0),
  },
  'foreign-table': {
    category: 'conservative',
    unread: 'relkind === "f"',
  ...
  'validated-check': {
    ...
    detect: (s) =>
      anyConstraint(
        s,
        (c) => c.type === 'check' && c.validated && !/^NOT NULL /.test(c.definition),
      ),
  },
  'not-valid-check': {
    category: 'gated',
    ...
    detect: (s) =>
      anyConstraint(
        s,
        (c) => c.type === 'check' && !c.validated && /NOT VALID/i.test(c.definition),
      ),
  },
  'not-enforced-check': {
    category: 'gated',
    ...
    detect: (s) =>
      anyConstraint(s, (c) => c.type === 'check' && /NOT ENFORCED/i.test(c.definition)),
  },
  ...
  'validated-single-column-foreign-key': {
    ...
    detect: (s) =>
      anyConstraint(
        s,
        (c) => c.type === 'foreign' && c.validated && !c.deferrable && c.columns.length === 1,
      ),
  },
  ...
  'not-enforced-foreign-key': {
    ...
    detect: (s) =>
      anyConstraint(s, (c) => c.type === 'foreign' && /NOT ENFORCED/i.test(c.definition)),
  },
  'primary-key': {
    category: 'conservative',
    unread: '"primaryKey"',
