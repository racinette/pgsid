...
describe('Q1: operator resolution under search_path', () => {
  it('gathers a user operator iff its schema is on the path — position irrelevant', async () => {
    ...
    expect(await errorOf('SELECT true + false')).toContain('operator does not exist')
  })
...
describe('Q2: aggregate and window signatures', () => {
  ...
  it('makes the two rank shapes mutually exclusive', async () => {
    ...
    expect((await rows('SELECT rank() OVER (ORDER BY val) AS r FROM tq2 LIMIT 1'))[0]!.r).toBe(1)
    expect(await errorOf('SELECT rank() FROM tq2')).toContain('requires an OVER clause')
    expect(await errorOf('SELECT rank(1) WITHIN GROUP (ORDER BY val) OVER () FROM tq2')).toContain(
  ...
  it("unifies a hypothetical call's direct arguments with the ORDER BY types", async () => {
    ...
    expect(await errorOf("SELECT rank('a') WITHIN GROUP (ORDER BY val) FROM tq2")).toContain(
      'invalid input syntax for type integer',
    )
    expect((await rows("SELECT rank('a') WITHIN GROUP (ORDER BY txt) AS r FROM tq2"))[0]!.r).toBe(1)
  })

  it('keeps the totality split the curated tables record: rank 1 over empty, percentile NULL', async () => {
    expect(
      (await rows('SELECT rank(1) WITHIN GROUP (ORDER BY val) AS r FROM tq2 WHERE false'))[0]!.r,
    ).toBe(1)
    expect(
      (
        await rows(
  ...
  it('accepts mixed untouched types through VARIADIC "any"', async () => {
    ...
    expect((await rows('SELECT concat(val, txt, iv) AS r FROM tq2 LIMIT 1'))[0]!.r).toBe(
      '1x01:00:00',
    )
  })
