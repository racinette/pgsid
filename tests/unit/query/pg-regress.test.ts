...
function scheduledOrder(): string[] {
  ...
  return [...ordered.filter((f) => all.includes(f)), ...all.filter((f) => !scheduled.has(f)).sort()]
}

describe.runIf(existsSync(SQL_DIR) && !!process.env.REGRESS_REPLAY)(
  'PostgreSQL regression corpus (shape and refusal replay)',
  () => {
    const tally = {
      files: 0,
      replayed: 0,
        ...
        for (const unit of units) {
          ...
          if (!result.ok) {
            ...
            if (process.env.REGRESS_TRACE_FILE === file) {
              console.error(
                `TRACE ${file}:${unit.line} FAIL ${result.error?.slice(0, 60)} :: ${unit.text.replace(/\s+/g, ' ').slice(0, 60)}`,
              )
            }
          ...
          if (process.env.REGRESS_TRACE_FILE === file) {
            console.error(
              `TRACE ${file}:${unit.line} ok fields=${JSON.stringify(result.fields)} :: ${unit.text.replace(/\s+/g, ' ').slice(0, 60)}`,
            )
          }
          ...
          const setsSearchPath =
            ...
            (stmt as { VariableSetStmt?: { name?: string } } | undefined)?.VariableSetStmt?.name ===
              'search_path'
