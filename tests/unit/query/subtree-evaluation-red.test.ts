...
describe('kernel atom oracle (flipped 2026-08-12 — convicted by crafted fixtures)', () => {
  it('a CHECK refutes a CASE guard through same-operand trichotomy', async () => {
    const c = await contract('SELECT CASE WHEN a <= 5 THEN NULL ELSE 5 END AS a2 FROM tri')
...
describe('interval exclusivity (flipped 2026-08-12 — the chartered rung landed)', () => {
  ...
  it('disjoint rays with room between anchors', async () => {
    ...
    expect(await notNullOf('SELECT CASE WHEN c.a <= 3 THEN NULL ELSE 5 END AS r FROM tri2 c')).toBe(
      true,
    )
  })
  ...
  it('adjacent integer anchors, no room between', async () => {
    ...
    // it today.)
    expect(await notNullOf('SELECT CASE WHEN c.a <= 4 THEN NULL ELSE 5 END AS r FROM tri2 c')).toBe(
      true,
    )
  })
  ...
  it('a point CHECK refutes rays that miss it, both directions', async () => {
    expect(await notNullOf('SELECT CASE WHEN c.p > 7 THEN NULL ELSE 5 END AS r FROM pt c')).toBe(
      true,
    )
    expect(await notNullOf('SELECT CASE WHEN c.p < 3 THEN NULL ELSE 5 END AS r FROM pt c')).toBe(
      true,
    )
  })
  ...
  it('a float ray survives its NaN rows', async () => {
    ...
    expect(await notNullOf('SELECT CASE WHEN c.f <= 3 THEN NULL ELSE 5 END AS r FROM flo c')).toBe(
      true,
    )
  })
  ...
  it('numeric anchors of different literal kinds still order', async () => {
    ...
    expect(await notNullOf('SELECT CASE WHEN c.n <= 5 THEN NULL ELSE 5 END AS r FROM nm c')).toBe(
      true,
    )
  })
  ...
  it('control: same-token exclusivity answers today, no anchors needed', async () => {
    ...
    expect(await notNullOf('SELECT CASE WHEN c.a < 5 THEN NULL ELSE 5 END AS r FROM tri c')).toBe(
      true,
    )
    expect(await notNullOf('SELECT CASE WHEN c.g < 5 THEN NULL ELSE 5 END AS r FROM ge c')).toBe(
      true,
    )
  })
  ...
  it('GUARD: overlapping rays claim nothing', async () => {
    ...
    expect(await notNullOf('SELECT CASE WHEN c.a <= 6 THEN NULL ELSE 5 END AS r FROM tri2 c')).toBe(
      false,
    )
  })
  ...
  it('GUARD: a point inside a closed ray claims nothing', async () => {
    expect(await notNullOf('SELECT CASE WHEN c.p >= 5 THEN NULL ELSE 5 END AS r FROM pt c')).toBe(
      false,
    )
  })
  ...
  it('GUARD: a complement excludes only its own point', async () => {
    ...
    expect(await notNullOf('SELECT CASE WHEN c.z = 3 THEN NULL ELSE 5 END AS r FROM ne2 c')).toBe(
      false,
    )
  })
  ...
  it("a DEFAULT-collated column's order anchors claim (flipped 2026-08-12)", async () => {
    ...
    expect(
      await notNullOf("SELECT CASE WHEN c.s <= 'k' THEN NULL ELSE 5 END AS r FROM stx c"),
    ).toBe(true)
  })
  ...
  it('the equality arm still works under an explicit collation', async () => {
    ...
    expect(
      await notNullOf("SELECT CASE WHEN c.s = 'beta' THEN NULL ELSE 5 END AS r FROM stxeq c"),
    ).toBe(true)
    // The own point fires — the boundary the ne relation must not cross.
    expect(
      await notNullOf("SELECT CASE WHEN c.s = 'alpha' THEN NULL ELSE 5 END AS r FROM stxeq c"),
    ).toBe(false)
  })
  ...
  it("GUARD: an explicitly-collated column's order stays refused", async () => {
    ...
    expect(
      await notNullOf("SELECT CASE WHEN c.s <= 'k' THEN NULL ELSE 5 END AS r FROM stxc c"),
    ).toBe(false)
  })
  ...
  it('ISO datetime anchors order (flipped 2026-08-16 — design B landed)', async () => {
    ...
    expect(
      await notNullOf("SELECT CASE WHEN c.d <= '2019-06-01' THEN NULL ELSE 5 END AS r FROM dtc c"),
    ).toBe(true)
    expect(
      await notNullOf("SELECT CASE WHEN c.d <= '1/2/2020' THEN NULL ELSE 5 END AS r FROM dtc c"),
    ).toBe(false)
  })
})
...
describe('the lossy anchor read (found & closed 2026-08-24)', () => {
  ...
  it('containment must not select an arm through one either', async () => {
    ...
    expect(await notNullOf('SELECT o FROM cain c WHERE c.a < 2.4')).toBe(false)
  })
...
describe('partition bounds (flipped 2026-08-16 — the chartered rung landed)', () => {
  ...
  it("a range bound refutes a guard's interval on a direct partition scan", async () => {
    ...
    expect(
      await notNullOf('SELECT CASE WHEN c.id >= 150 THEN NULL ELSE 5 END AS r FROM prt_lo c'),
    ).toBe(true)
  })
  ...
  it('GUARD: bounds never leak to a parent scan', async () => {
    ...
    expect(
      await notNullOf('SELECT CASE WHEN c.id >= 150 THEN NULL ELSE 5 END AS r FROM prt c'),
    ).toBe(false)
  })
  ...
  it('GUARD: an overlapping guard claims nothing from the bound', async () => {
    ...
    expect(
      await notNullOf('SELECT CASE WHEN c.id >= 50 THEN NULL ELSE 5 END AS r FROM prt_lo c'),
    ).toBe(false)
  })
  ...
  it('a range partition under a HASH parent feeds; the hash conjunct is inert', async () => {
    ...
    expect(
      await notNullOf('SELECT CASE WHEN t.id >= 150 THEN NULL ELSE 5 END AS r FROM hn_0_lo t'),
    ).toBe(true)
    expect(
      await notNullOf('SELECT CASE WHEN t.id >= 50 THEN NULL ELSE 5 END AS r FROM hn_0_lo t'),
    ).toBe(false)
...
describe('list membership exclusion (flipped 2026-08-16 — the rung landed)', () => {
  ...
  it('a CHECK IN-list refutes an outside point — every member excludes it', async () => {
    expect(await notNullOf("SELECT CASE WHEN c.k = 'q' THEN NULL ELSE 5 END AS r FROM lme c")).toBe(
      true,
    )
  })

  it('an integer IN-list refutes an outside point and an outside ray', async () => {
    expect(await notNullOf('SELECT CASE WHEN c.n = 9 THEN NULL ELSE 5 END AS r FROM lmei c')).toBe(
      true,
    )
    expect(await notNullOf('SELECT CASE WHEN c.n <= 0 THEN NULL ELSE 5 END AS r FROM lmei c')).toBe(
      true,
    )
  })
  ...
  it('the list-partition twin refutes through the same code', async () => {
    expect(
      await notNullOf("SELECT CASE WHEN c.k = 'q' THEN NULL ELSE 5 END AS r FROM plst_ab c"),
    ).toBe(true)
  })

  it('GUARD: a guard naming a MEMBER still fires', async () => {
    // Row ('a','x') fires the arm — NULL witnessed.
    expect(await notNullOf("SELECT CASE WHEN c.k = 'a' THEN NULL ELSE 5 END AS r FROM lme c")).toBe(
      false,
    )
  })
  ...
  it('GUARD: an OR-fact with one non-refuting arm claims nothing', async () => {
    ...
    expect(
      await notNullOf("SELECT CASE WHEN c.k = 'q' THEN NULL ELSE 5 END AS r FROM lmor c"),
    ).toBe(false)
  })
  ...
  it('GUARD: the NULL-listing bound shape still claims nothing', async () => {
    ...
    expect(
      await notNullOf("SELECT CASE WHEN c.k = 'q' THEN NULL ELSE 5 END AS r FROM plst_n c"),
    ).toBe(false)
  })
})
...
describe('guard-side IN (flipped 2026-08-16 — the rung landed)', () => {
  ...
  it("an IN guard reaches the OR spelling's conclusion", async () => {
    expect(
      await notNullOf("SELECT CASE WHEN c.k IN ('q','r') THEN NULL ELSE 5 END AS r FROM lme c"),
    ).toBe(true)
  })
  ...
  it('the list-partition twin refutes an IN guard through the same code', async () => {
    expect(
      await notNullOf("SELECT CASE WHEN c.k IN ('q','r') THEN NULL ELSE 5 END AS r FROM plst_ab c"),
    ).toBe(true)
  })
  ...
  it('GUARD: NOT IN is a conjunction and must not ride the rule', async () => {
    ...
    expect(
      await notNullOf("SELECT CASE WHEN c.k NOT IN ('q','r') THEN NULL ELSE 5 END AS r FROM lme c"),
    ).toBe(false)
  })
  ...
  it('GUARD: a list naming one MEMBER stays unrefuted', async () => {
    ...
    // witnesses the NULL.
    expect(
      await notNullOf("SELECT CASE WHEN c.k IN ('a','q') THEN NULL ELSE 5 END AS r FROM lme c"),
    ).toBe(false)
  })
  ...
  it("GUARD: a NULL in the guard's list refuses the whole desugar", async () => {
    ...
    expect(
      await notNullOf("SELECT CASE WHEN c.k IN ('q', NULL) THEN NULL ELSE 5 END AS r FROM lme c"),
    ).toBe(false)
  })
})
...
describe('closed sublinks (flipped 2026-08-16 — the rung landed)', () => {
  ...
  it('a closed EXPR sublink answers and the guard prunes', async () => {
    expect(
      await notNullOf('SELECT CASE WHEN (SELECT 7) = 7 THEN o.id ELSE NULL END AS c FROM orders o'),
    ).toBe(true)
  })
...
describe('GUARD: lines the mechanism must not cross', () => {
  ...
  it('a closed ON condition does not touch join semantics', async () => {
    const c = await contract('SELECT o2.qty AS q FROM orders o LEFT JOIN orders o2 ON TRUE')
