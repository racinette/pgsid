...
describe('spike: body offset strategy verification', () => {
  ...
  it('AST body offset: $$ delimiter', async () => {
    const sql =
      'CREATE FUNCTION public.dollar_def(a int) RETURNS int LANGUAGE plpgsql AS $$\nBEGIN\n  RETURN a;\nEND;\n$$;'
  ...
  it('AST body offset: $tag$ delimiter', async () => {
    const sql =
      'CREATE FUNCTION public.dollar_tag(a int) RETURNS int LANGUAGE plpgsql AS $tag$\nBEGIN\n  RETURN a;\nEND;\n$tag$;'
    const buf = Buffer.from(sql, 'utf8')
    const parsed = await parseSql(sql)
    const stmt = parsed.stmts![0]!.stmt!
    const stmtStart = parsed.stmts![0]!.stmt_location ?? 0
  ...
  it('AST body offset: $some_arbitrary_string$ delimiter', async () => {
    const sql =
      'CREATE FUNCTION public.dollar_arb(a int) RETURNS int LANGUAGE plpgsql AS $some_arbitrary_string$\nBEGIN\n  RETURN a;\nEND;\n$some_arbitrary_string$;'
    const buf = Buffer.from(sql, 'utf8')
    const parsed = await parseSql(sql)
    const stmt = parsed.stmts![0]!.stmt!
    const stmtStart = parsed.stmts![0]!.stmt_location ?? 0
  ...
  it('AST body offset: with comments before AS', async () => {
    const sql =
      'CREATE FUNCTION public.with_comments(a int) -- line\nRETURNS int /* block */ LANGUAGE plpgsql AS $$\nBEGIN\n  RETURN a;\nEND;\n$$;'
    const buf = Buffer.from(sql, 'utf8')
    const parsed = await parseSql(sql)
    const stmt = parsed.stmts![0]!.stmt!
    const stmtStart = parsed.stmts![0]!.stmt_location ?? 0
  ...
  it('AST body offset: multi-byte UTF-8 before body', async () => {
    const sql =
      'CREATE FUNCTION public.utf8_body(a int) RETURNS int -- café ☕ 日本語\nLANGUAGE plpgsql AS $$\nBEGIN\n  RETURN a;\nEND;\n$$;'
    const buf = Buffer.from(sql, 'utf8')
    const parsed = await parseSql(sql)
    const stmt = parsed.stmts![0]!.stmt!
    const stmtStart = parsed.stmts![0]!.stmt_location ?? 0
  ...
  it('AST offset and byte-search offset agree for CreateFunctionStmt', async () => {
    const sql =
      'CREATE FUNCTION public.agree_test(a int) RETURNS int LANGUAGE plpgsql AS $$\nBEGIN\n  RETURN a;\nEND;\n$$;'
    const buf = Buffer.from(sql, 'utf8')
    const parsed = await parseSql(sql)
    const stmt = parsed.stmts![0]!.stmt!
    const stmtStart = parsed.stmts![0]!.stmt_location ?? 0
  ...
  it('AST offset and byte-search offset agree for single-quoted body', async () => {
    const sql =
      "CREATE FUNCTION public.agree_single(a int) RETURNS int LANGUAGE sql AS 'SELECT a + 1';"
    const buf = Buffer.from(sql, 'utf8')
    const parsed = await parseSql(sql)
    const stmt = parsed.stmts![0]!.stmt!
    const stmtStart = parsed.stmts![0]!.stmt_location ?? 0
