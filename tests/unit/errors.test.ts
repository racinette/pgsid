...
import { parseSql, preprocess, stripConcurrently, mapStrippedToOriginal } from '../../src/ast.js'
...
describe('errors: extractExecDiagnostic', () => {
  ...
  async function prepareAndExtract(
    ...
    try {
      ...
      try {
        await pg.query(`DEALLOCATE ${name}`)
      } catch {
        /* ignore */
      }
    }
  ...
  async function checkFunction(
    ...
    try {
      ...
      return res.rows.map((row) =>
        extractPlpgsqlCheckDiagnostic(row as PlpgsqlCheckRow, bodyOffset, bodyText),
      )
    } finally {
      await pg.query('ROLLBACK')
    }
