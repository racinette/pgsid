...
describe('diffCatalogs: empty before → everything added', () => {
  it('reports all entities as added on first boot', () => {
    ...
    expect(diff.added).toEqual(['public', 'public.status', 'public.users', 'public.users.id'])
  })
})
...
describe('diffCatalogs: tables', () => {
  ...
  it('storage param change → table modified (columns unaffected)', () => {
    const before = snapshot({
      tables: [
        table('public', 't', [col({ name: 'id' })], { storageParams: { fillfactor: '80' } }),
      ],
    })
    const after = snapshot({
      tables: [
        table('public', 't', [col({ name: 'id' })], { storageParams: { fillfactor: '90' } }),
      ],
    })
    const diff = diffCatalogs(before, after)
    expect(diff.added).toEqual([])
  ...
  it('constraint change → table modified', () => {
    ...
    const before = snapshot({
      tables: [table('public', 't', [col({ name: 'id' })], { constraints: [c1] })],
    })
    const after = snapshot({
      tables: [table('public', 't', [col({ name: 'id' })], { constraints: [c2] })],
    })
    const diff = diffCatalogs(before, after)
    expect(diff.modified.map((m) => m.entityId)).toEqual(['public.t'])
  })
  ...
  it('partition bound change → table modified; unchanged bound → no diff', () => {
    ...
    const b2: TableInfo['partitionBound'] = {
      ...b1,
      definition: '((id IS NOT NULL) AND (id >= 0) AND (id < 200))',
    }
...
describe('diffCatalogs: columns', () => {
  ...
  it('collation change → column modified', () => {
    ...
    expect(diffCatalogs(before, afterIdentity).modified.map((m) => m.entityId)).toEqual([
      'public.users.handle',
    ])
    expect(diffCatalogs(before, afterDeterminism).modified.map((m) => m.entityId)).toEqual([
      'public.users.handle',
    ])
  ...
  it('DEFAULT change → column modified', () => {
    const before = snapshot({
      tables: [
        table('public', 'users', [
          col({ name: 'created_at', hasDefault: false, defaultExpr: null }),
        ]),
      ],
    })
    const after = snapshot({
      tables: [
        table('public', 'users', [
          col({ name: 'created_at', hasDefault: true, defaultExpr: 'now()' }),
        ]),
      ],
    })
    const diff = diffCatalogs(before, after)
...
describe('diffCatalogs: functions', () => {
  ...
  it('return type change → function modified', () => {
    const before = snapshot({
      functions: [fn('public', 'calc', 'integer', { returnType: 'integer' })],
    })
    const after = snapshot({
      functions: [fn('public', 'calc', 'integer', { returnType: 'text', returnTypeOid: 25 })],
    })
    const diff = diffCatalogs(before, after)
  ...
  it('volatile change → function modified', () => {
    const before = snapshot({
      functions: [fn('public', 'calc', 'integer', { volatile: 'volatile' })],
    })
    const after = snapshot({
      functions: [fn('public', 'calc', 'integer', { volatile: 'immutable' })],
    })
    const diff = diffCatalogs(before, after)
    expect(diff.modified.map((m) => m.entityId)).toEqual(['public.calc(integer)'])
  })
  ...
  it('body-only change → NOT modified (signature unchanged)', () => {
    ...
    const after = snapshot({
      functions: [fn('public', 'calc', 'integer', { body: 'new body', definition: 'new def' })],
    })
    const diff = diffCatalogs(before, after)
    expect(diff.modified).toEqual([])
  })
...
describe('diffCatalogs: views + materialized views', () => {
  ...
  it('added view emits view + columns as added', () => {
    ...
    const after = snapshot({
      views: [
        view('public', 'v', [col({ name: 'id' }), col({ name: 'name' })], 'SELECT id, name FROM t'),
      ],
    })
  ...
  it('definition change → view modified, columns unaffected', () => {
    const before = snapshot({
      views: [view('public', 'v', [col({ name: 'id' })], 'SELECT id FROM t')],
    })
    const after = snapshot({
      views: [view('public', 'v', [col({ name: 'id' })], 'SELECT id FROM t WHERE true')],
    })
    const diff = diffCatalogs(before, after)
    ...
    const before = snapshot({
      views: [
        view(
          'public',
          'v',
          [col({ name: 'id', typeOid: 23, typeName: 'integer' })],
          'SELECT id FROM t',
        ),
      ],
    })
    const after = snapshot({
      views: [
        view(
          'public',
          'v',
          [col({ name: 'id', typeOid: 20, typeName: 'bigint' })],
          'SELECT id FROM t',
        ),
      ],
    })
  ...
  it('materialized view columns are tracked like view columns', () => {
    const before = snapshot()
    const after = snapshot({
      materializedViews: [view('public', 'mv', [col({ name: 'id' })], 'SELECT id FROM t')],
    })
...
describe('diffCatalogs: indexes', () => {
  ...
  it('added index → added', () => {
    expect(diffCatalogs(snapshot(), snapshot({ indexes: [idx('public', 'i')] })).added).toEqual([
      'public.i',
    ])
  })
...
describe('diffCatalogs: composite types, sequences, extensions, schemas', () => {
      ...
      compositeTypes: [
        {
          schema: 'public',
          name: 'addr',
          attributes: [{ name: 'city', typeOid: 25, typeName: 'text' }],
        },
      ],
    })
    const after = snapshot({
      compositeTypes: [
        {
          schema: 'public',
          name: 'addr',
          attributes: [{ name: 'city', typeOid: 23, typeName: 'integer' }],
        },
      ],
    })
  ...
  it('extension version change → extension modified (id is name, no schema)', () => {
    const ext = (version: string): ExtensionInfo => ({
      name: 'plpgsql_check',
      version,
      schema: 'public',
    })
