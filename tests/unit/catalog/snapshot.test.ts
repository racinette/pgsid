...
describe('snapshotCatalog: empty schema', () => {
  ...
  it('returns a well-formed (empty-of-user-objects) snapshot for a fresh public schema', async () => {
    ...
    expect(s.extensions.map((x) => x.name)).toEqual(
      expect.arrayContaining(['plpgsql', 'plpgsql_check']),
    )
...
describe('snapshotCatalog: functions and procedures', () => {
  ...
  it('captures plpgsql/sql functions and procedures with args, return, volatility', async () => {
    ...
    expect(add?.args[0]).toMatchObject({
      name: 'a',
      mode: 'in',
      typeOid: 23,
      typeName: 'integer',
  ...
  it('captures the polymorphic ARRAY signatures, with their argument types', async () => {
    ...
    expect(byName('array_agg')).toEqual(['anyarray -> anyarray', 'anynonarray -> anyarray'])
    ...
    for (const sig of s.builtinPolymorphicArraySignatures) {
      expect(
        sig.args.some((a) => POLY.has(a)),
        `${sig.name}(${sig.args.join(', ')})`,
      ).toBe(true)
  ...
  it('captures the claim-table signatures, scoped both ways', async () => {
    ...
    const fnNames = new Set([
      ...
      ...STRICT_TOTAL_BUILTINS,
      ...NON_NULL_OVER_NONEMPTY_AGGREGATES,
      ...[...NEVER_NULL_WINDOW_SIGNATURES, ...STRICT_TOTAL_WINDOW_SIGNATURES].map((k) =>
        k.slice(0, k.indexOf('(')),
      ),
  ...
  it('carries the resolution keys the overload measurements established', async () => {
    ...
    expect(
      rank.map(({ kind, aggKind, numDirectArgs, variadic, args }) => ({
        kind,
        aggKind,
        numDirectArgs,
        variadic,
    ...
    expect(
      s.builtinOperatorSignatures.some((sig) => sig.name === '-' && sig.leftType === null),
    ).toBe(true)
  ...
  it('captures per-signature volatility over ALL of pg_catalog', async () => {
    ...
    expect(dp.find((f) => f.args.join(',') === 'text,timestamp with time zone')).toMatchObject({
      volatility: 's',
    })
    ...
    expect(
      s.builtinOperatorVolatilities.some(
  ...
  it('includes extension functions (plpgsql_check) but they are not validated', async () => {
    const s = await snapshotCatalog(pg)
    const extFns = s.functions.filter(
      (f) => f.name === 'plpgsql_check_function_tb' || f.name === 'plpgsql_check_function',
    )
...
describe('snapshotCatalog: integration with migration fixtures + diff', () => {
  ...
  it('captures the 0001+0002+0003 migration set', async () => {
    ...
    expect(s.indexes.find((i) => i.name === 'posts_published_idx')?.partial).toContain(
