...
describe('BUG FIXES: silent skips and missing provenance', () => {
  ...
  it('FIX 3a: pre-existing function is skipped (not our responsibility)', async () => {
    ...
    const preDiags = diags.filter(
      (d) => d.message.includes('nonexistent_fix3') || d.message.includes('preexist_fix3'),
    )
  ...
  it('FIX 3b: unknown-origin function → engine warning (not silent skip)', async () => {
    ...
    const unknownDiags = diags.filter(
      (d) => d.message.includes('unknown_origin_fn') || d.message.includes('provenance'),
    )
