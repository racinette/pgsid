...
describe('preprocess: stripConcurrently', () => {
  ...
  it('handles non-ASCII comments before the keyword (latin1 scan)', async () => {
    ...
    expect(result.modified).toBe(true)
    expect(result.content.toString('utf8')).toBe('-- café ☕\nCREATE INDEX u_idx ON t (id);')
  })
})
...
describe('preprocess: StatementContext', () => {
  ...
  it('ctx.length excludes trailing ; and whitespace', async () => {
    ...
    const first = seen[0]!
    expect(
      buf.subarray(first.sourceOffset, first.sourceOffset + first.length).toString('utf8'),
    ).toBe('CREATE TABLE t (id int)')

    // Last stmt: "SELECT 1" — no trailing ; or whitespace.
    const second = seen[1]!
    expect(
      buf.subarray(second.sourceOffset, second.sourceOffset + second.length).toString('utf8'),
    ).toBe('SELECT 1')
  })
})
...
function filterForScenario(scenario: string): StatementFilter {
  switch (scenario) {
    case 'strip_concurrently':
      return stripConcurrently()
    default:
      throw new Error(`unknown scenario: ${scenario}`)
  }
}

function discoverFixtureCases(): {
  name: string
  input: Buffer
  expected: Buffer
  filter: StatementFilter
...
describe('remapping: realistic position-mapping scenarios', () => {
  it('CONCURRENTLY removal: maps a position after the keyword correctly', async () => {
    ...
    expect(
      input.toString('utf8').slice(originalIdxPos, originalIdxPos + 'users_email_idx'.length),
