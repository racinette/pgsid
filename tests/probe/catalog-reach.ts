...
const relationScoped = Object.keys(FEATURES).filter((k) => {
  try {
    return !FEATURES[k]!.detect(empty, env)
  } catch {
    return false
  }
})
const detectOn = (snap: CatalogSnapshot, k: string): boolean => {
  try {
    return FEATURES[k]!.detect(snap, env)
  } catch {
    return false
  }
}
...
console.log(
  `  relation-scoped but absent (census 'absent' markers): ${notInSchema.length} — ${notInSchema.join(', ')}`,
)
...
console.log(
  `relations a generated query names: ${queried.length} — ${queried.map((r) => r.id).join(', ')}`,
)
for (const r of queried)
  console.log(
    `    ${r.id.padEnd(14)} ${[...r.features].sort().join(', ') || '(nothing classified)'}`,
  )
const profileKey = (r: (typeof rels)[number]) => [...r.features].sort().join('|')
console.log(
  `distinct catalog profiles: ${new Set(rels.map(profileKey)).size} across the schema, ` +
...
for (const x of rels
  ...
  .slice(0, 15)) {
  console.log(
    `  ${String(x.gain.length).padStart(2)}  ${x.r.id.padEnd(24)} [${x.r.kind}]  ${x.gain.join(', ')}`,
  )
}
...
for (let rank = 1; ; rank++) {
  ...
  console.log(
    `${String(rank).padStart(2)}. ${best.id.padEnd(24)} [${best.kind}] +${bestNew.length}  ${bestNew.join(', ')}`,
  )
...
for (const t of s.tables)
  for (const c of t.columns) notNullCol.set(`${t.schema}.${t.name}.${c.name}`, c.notNull)
const edges: { child: string; parent: string; col: string; verdict: string }[] = []
for (const t of s.tables) {
  for (const c of t.constraints) {
    if (c.type !== 'foreign' || c.columns.length !== 1 || !c.foreignSchema || !c.foreignTable)
      continue
    ...
    edges.push({
      child,
      parent: `${c.foreignSchema}.${c.foreignTable}`,
      col: c.columns[0]!,
      verdict: !c.validated
        ? 'yes — key is NOT VALID'
        : nn
          ? 'only joining parent -> child'
          : 'yes — key column is nullable',
    })
  }
}
...
for (const e of edges.sort((a, b) => (a.child + a.col < b.child + b.col ? -1 : 1))) {
  console.log(
    `  ${(e.child + '.' + e.col).padEnd(38)} -> ${e.parent.padEnd(20)} NULL-extended row possible: ${e.verdict}`,
  )
}
...
console.log(
  `  no single-column key in either direction, so nothing joins to them: ${[...comp.values()].filter((c) => c.length === 1).length} tables`,
)

const inComponent = new Set(joined.flat())
const onlyIsolated = unreached.filter(
  (f) => !rels.some((r) => inComponent.has(r.id) && r.features.has(f)),
)
console.log(`\nof the ${unreached.length} unreachable features:`)
console.log(
  `  ${unreached.length - onlyIsolated.length} are on a table inside one of those joinable groups — ` +
    unreached.filter((f) => !onlyIsolated.includes(f)).join(', '),
)
console.log(
  `  ${onlyIsolated.length} are ONLY on tables no key connects to anything — ${onlyIsolated.join(', ')}`,
)
