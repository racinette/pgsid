...
function edgesOf(snapshot: CatalogSnapshot, childToParent: Map<string, string>): Edge[] {
  ...
  const push = (
    ...
    out.push({
      ...
      childToParentCanExtend: !enforced || !c.validated || cols.some((x) => !x!.notNull),
    })
  }
...
function projectionExpr(
  ...
  const pair = (want?: string): [Slot, Slot] | null => {
    const buckets = want ? (byType.has(want) ? [byType.get(want)!] : []) : [...byType.values()]
  ...
  switch (rand.pick(forms)) {
    ...
    case 'coalesce': {
      const p = pair()
      return p
        ? { ast: { CoalesceExpr: { args: [ref(p[0]), ref(p[1])] } }, form: 'coalesce' }
        : null
    }
    ...
    case 'array': {
      const p = pair()
      return p
        ? { ast: { A_ArrayExpr: { elements: [ref(p[0]), ref(p[1])] } }, form: 'array' }
        : null
    }
...
function predicate(rand: Rand, used: Item[], pool: ValuePool, pr?: ParamRefFn): Ast | null {
  ...
  const values = (u.table ? (pool.get(`${u.table}.${col.name}`) ?? []) : []).filter(
    (v) => v !== null,
  )
...
): Map<string, string> => new Map(catalog.map((c, i) => [c.name, effective[i]?.name ?? c.name]))
...
const seriesCall = (lo: number, hi: number): Ast => ({
  FuncCall: {
    funcname: [str('generate_series')],
    args: [intConst(lo), intConst(hi)],
    funcformat: 'COERCE_EXPLICIT_CALL',
  },
})
...
function relationItem(
  ...
  if (roll === 0) {
    return {
      ast: {
        RangeTableSample: {
          relation: rangeVar(t.schema, t.name, alias),
          method: [str('bernoulli')],
          args: [intConst(rand.int(60, 100))],
        },
      },
      form: 'sample',
      columns: cols,
      ...
      ast: rangeSubselect(
        {
          SelectStmt: {
            ...
            fromClause: [rangeVar(t.schema, t.name, `${alias}_b`)],
...
function buildQuery(
  ...
  const colsOf = (t: TableInfo) =>
    t.columns
      .filter((c) => c.generated !== 'virtual')
      .map((c) => ({ name: c.name, type: c.typeName }))
  const P = makeParams()
    ...
    for (const u of used) {
      ...
      for (const c of u.columns) {
        for (const d of colsOf(t)) {
          if (c.type === d.type && c.type !== 'json')
            options.push({ anchor: u, a: c.name, b: d.name })
        }
      }
    }
  ...
  for (let i = 1; i < target_; i++) {
    ...
    if (roll === 0) {
      ...
      used.push({
        alias,
        columns: cols.map((n) => ({ name: n, type: 'integer' })),
        renamed: new Map(),
      })
      ...
      from = {
        JoinExpr: {
          ...
          rarg: rangeSubselect(
            {
              SelectStmt: {
                targetList: [target(colRef(anchor.alias, c.name), 'lv')],
                limitOption: 'LIMIT_OPTION_DEFAULT',
                op: 'SETOP_NONE',
              },
            },
      ...
      from = {
        JoinExpr: {
          ...
          ...(pair
            ? {
                quals: eq(
                  colRef(pair.anchor.alias, pair.a),
                  colRef(alias, item.columns[colsOf(t).findIndex((x) => x.name === pair.b)]!.name),
                ),
              }
            : {}),
        },
    ...
    const candidates = edges.filter(
      (e) => inTree.some((u) => u.table === e.child) || inTree.some((u) => u.table === e.parent),
    )
    ...
    const conjuncts2 = anchorCols.map((c, k) =>
      eq(colRef(anchor.alias, anchor.renamed.get(c) ?? c), colRef(alias, renamedTo(addedCols[k]!))),
    )
    ...
    treeAliases.add(alias)
    shapeParts.push(
      `${kind}:${addParent ? 'child->parent' : 'parent->child'}${item.form === 'rel' ? '' : `/${item.form}`}`,
    )
  }
  ...
  if (rand.chance(0.15)) {
    ...
    const orderable = colsOf(t).filter(
      (x) => !['boolean', 'json', 'jsonb'].includes(x.type) && !x.type.endsWith('[]'),
    )
    if (orderable.length === 0) return null
    ...
    if (p) {
      ...
      if (rand.chance(0.4)) {
        const cands = colsOf(t)
          .map((c) => ({
            c,
            vs: (pool.get(`${t.schema}.${t.name}.${c.name}`) ?? []).filter(
              (x) => x !== null && literalFor(x) !== null,
            ),
          }))
          .filter((x) => x.vs.length > 0)
        if (cands.length) {
          const pick = rand.pick(cands)
          inner = boolExpr('AND_EXPR', [
            corr,
            eq(colRef('ex', pick.c.name), P.ref(rand.pick(pick.vs), 'exists-cmp')),
          ])
        }
      }
      existsPred = {
  ...
  return {
    ...
    shape:
      `${used.map((u) => u.table ?? 'fn').join('+')}|${shapeParts.join(',')}|W:${where?.shape ?? '-'}` +
...
function buildMerge(
  ...
  for (const v of [...existing].slice(0, rand.int(1, 3))) {
    ...
    // MERGE USING sources.
    sourceKeys.push(
      castableType(keyInfo.typeName) && rand.chance(0.3)
  ...
  if (!routed && rand.chance(0.8)) {
    ...
    if (keyFk?.foreignTable && keyFk.foreignColumns?.length === 1) {
      ...
      const free = (pool.get(`${parentId}.${keyFk.foreignColumns[0]}`) ?? []).filter(
        (v) => v !== null && !taken.has(String(v)),
      )
  ...
  const setClause = (): Ast[] | null => {
    ...
    return v
      ? [{ ResTarget: { name: c.name, val: maybeParam(rand, v, P.ref, 'merge-set', 0.3) } }]
      : null
  }
  ...
  const addArm = (
    ...
    armForms.push(
      `${matchKind.replace('MERGE_WHEN_', '').toLowerCase()}:${commandType.replace('CMD_', '').toLowerCase()}`,
    )
  }
  ...
  if (straddles && rand.chance(0.8)) {
    ...
    for (const c of writable) {
      ...
      const v: WrittenValue | null =
        base[c.name] === null || base[c.name] === undefined
          ? c.notNull
            ? writtenValue(rand, tableId, c, pool, false)
            : { ast: nullConst(), value: null }
          : lit
            ? { ast: lit, value: base[c.name] }
            : null
      if (!v) {
  ...
  const stmt = {
    MergeStmt: {
      ...
      sourceRelation: rangeSubselect(
        {
          SelectStmt: {
            valuesLists: sourceKeys.map((k) => ({ List: { items: [k] } })),
            limitOption: 'LIMIT_OPTION_DEFAULT',
            op: 'SETOP_NONE',
          },
        },
  ...
  ;(
    (stmt.MergeStmt.sourceRelation as Record<string, any>)['RangeSubselect'] as Record<
      string,
      unknown
    >
  )['alias'] = { aliasname: 'ms', colnames: [str('k')] }
  ...
  return {
    ...
    kinds: [],
    shape:
      `${tableId}|MERGE:${[...armForms].sort().join(',')}${straddles ? '+straddle' : ''}${ret ? '+RET' : ''}` +
      `|P:${[...new Set(P.list.map((p) => p.site))].sort().join(',') || '-'}`,
  }
}
...
function aggregateTarget(rand: Rand, slots: Slot[], i: number): Ast {
  const numeric = slots.filter((sl) =>
    ['integer', 'bigint', 'smallint', 'numeric', 'double precision'].includes(sl.type),
  )
    ...
    return target(
      {
        FuncCall: {
          funcname: [str('sum')],
          args: [colRef(sl.alias, sl.column)],
          funcformat: 'COERCE_EXPLICIT_CALL',
        },
      },
      `a${i}_sum`,
  ...
  const sl =
    rand.pick(
      slots.filter((x) => !['json', 'jsonb', 'boolean'].includes(x.type) && !x.type.endsWith('[]')),
    ) ?? rand.pick(slots)
      ...
      : kind === 1
        ...
        : kind === 2
          ? {
              FuncCall: {
                funcname: [str('lag')],
                args: [colRef(ord.alias, ord.column)],
                over,
                funcformat: 'COERCE_EXPLICIT_CALL',
              },
            }
...
function decorate(
  ...
  if (slots.length > 0 && rand.chance(0.18)) {
    ...
    const tl: Ast[] = groupCols.map((sl, i) =>
      target(colRef(sl.alias, sl.column), `g${i}_${sl.column}`),
    )
  ...
  if (allInner && !grouped && !windowed && !sel['distinctClause'] && rand.chance(0.05)) {
    sel['lockingClause'] = [
      {
        LockingClause: {
          strength: rand.pick(['LCS_FORUPDATE', 'LCS_FORSHARE']),
...
function setOperation(
  rand: Rand,
  sel: Record<string, unknown>,
): { sel: Record<string, unknown>; form: string } {
...
function writtenValue(
  rand: Rand,
  tableId: string,
  col: { name: string; typeName: string; notNull: boolean },
  pool: ValuePool,
  fresh: boolean,
): WrittenValue | null {
...
function buildDml(
  ...
  if (kind === 0 || kind === 1) {
    ...
    for (const c of writable) {
      ...
      if (c.hasDefault && rand.chance(0.2)) {
        cols.push(c.name)
        vals.push(setToDefault())
        continue
      }
              ...
              ? [
                  {
                    ResTarget: {
                      name: c.name,
                      val: maybeParam(rand, v, P.ref, 'onconflict-set', 0.3),
                    },
                  },
                ]
              : []
    ...
    const onConflict = wantConflict
      ? {
          onConflictClause: {
            ...
            infer: {
              indexElems: keyCon!.columns.map((n) => ({
                IndexElem: {
                  name: n,
  ...
  if (kind === 2 || kind === 3) {
    ...
    if (kind === 3 && settable.length >= 2) {
          ...
          subselect: {
            SelectStmt: {
              targetList: [
                {
                  ResTarget: {
                    val: typeCast(maybeParam(rand, va, P.ref, 'multiassign', 0.3), a.typeName),
                  },
                },
                {
                  ResTarget: {
                    val: typeCast(maybeParam(rand, vb, P.ref, 'multiassign', 0.3), b.typeName),
                  },
                },
              ],
              limitOption: 'LIMIT_OPTION_DEFAULT',
      ...
      if (v.value !== null && rand.chance(0.35)) {
        const v2 =
          c.notNull && rand.chance(0.4) ? writtenValue(rand, tableId, c, pool, false) : null
        if (v2 && v2.value !== null) {
          ...
          val = {
            CoalesceExpr: {
              args: [P.ref(v.value, 'coalesce-set'), P.ref(v2.value, 'coalesce-set')],
            },
          }
...
const TIER: Record<Bucket, 'TOOL' | 'BUDGET' | 'FINDING' | 'EXPECTED' | 'OK'> = {
  ...
  'engine-crashed': 'FINDING',
  'shape-mismatch': 'FINDING',
  'notnull-violated': 'FINDING',
  ...
  'group-violated': 'FINDING',
  'parity-broke': 'FINDING',
  'agreed-rows': 'OK',
  'agreed-norows': 'OK',
}
  ...
  if (a && b && typeof a === 'object' && typeof b === 'object') {
    ...
    for (const k of new Set([...Object.keys(ao), ...Object.keys(bo)])) {
      ...
      if (JSON.stringify(ao[k]) !== JSON.stringify(bo[k]))
        return firstDiff(ao[k], bo[k], `${path}.${k}`)
    }
    return path
  }
...
function classify(r: ProbeResult): Bucket {
  if (r.error)
    return r.error.startsWith('UnsupportedNodeError') ? 'engine-refused' : 'engine-crashed'
  if (r.pgError) {
...
function fingerprint(bucket: Bucket, r: ProbeResult, built: Built): string {
  switch (bucket) {
    ...
    case 'shape-mismatch': {
      ...
      const engine = r.engineColumns.length,
        pg = r.pgColumns.length
    ...
    case 'param-violated': {
      ...
      const msgs = [
        ...new Set(
          r.paramViolations.map((v) =>
            normalise(v.replace(/^\$\d+ claimed nullable, binding NULL raised: /, '')),
          ),
        ),
      ].sort()
...
for (const t of snapshot.tables) {
  try {
    ...
    /* unreadable relation contributes no rows */
  }
}
...
const appTableInfos = snapshot.tables.filter(
  (t) => APPLICATION_TABLES.includes(t.name) && t.relkind !== 'p',
)
...
const usableEdges = edges.filter(
  (e) => inGroups.has(e.child) && inGroups.has(e.parent) && isApp(e.child) && isApp(e.parent),
)

console.log(`discovery slice 1 — seed ${SEED}, ${COUNT} queries`)
console.log(
  `join graph: ${usableEdges.length} single-column foreign keys over ` +
    `${groups.length} groups of joinable tables (largest ${groups[0]!.length})`,
)
console.log(
  `  child->parent outer joins that can null-extend: ` +
    usableEdges
      .filter((e) => e.childToParentCanExtend)
      .map((e) => `${e.child}.${e.childColumns.join('+')}`)
      .join(', '),
)
...
for (let i = 0; i < COUNT; i++) {
  ...
  try {
    ...
    rejectionDetail.set(
      `generator: ${(e as Error).message}`,
      (rejectionDetail.get(`generator: ${(e as Error).message}`) ?? 0) + 1,
    )
    continue
  }
  if (!built) {
  ...
  if (i > 0 && i % 1000 === 0) {
    curve.push(shapes.size - lastMark)
    lastMark = shapes.size
  }
  ...
  if (JSON.stringify(before) !== JSON.stringify(after)) {
    ...
    if (process.env.DISCOVERY_SHOW_REPARSE)
  ...
  if (TIER[bucket] === 'FINDING') {
    const detail =
      r.violations.join('; ') ||
      r.paramViolations.join('; ') ||
      r.groupViolations.join('; ') ||
      r.shape ||
      r.parity ||
      r.error ||
      ''
...
console.log(
  `  tables in the join graph:   ${inGroups.size} (the rest of the schema declares no single-column key, by design)`,
)
console.log(
  `  return rate:                ${returnable ? Math.round((returned / returnable) * 100) : 0}% (${returned}/${returnable} queries returned a row)`,
)
...
if (curve.length > 1) {
  ...
  console.log(
    `    marginal yield: ${Math.round(last)}/1000 at the end against ` +
      `${curve[0]} at the start (${Math.round((last / curve[0]!) * 100)}%)`,
  )
}
console.log(
  `  bound: 2..4 FROM items, left-deep — tables, derived tables, function items, LATERAL,`,
)
...
console.log(
  `         WHERE on ${pct(COUNT - (whereUse.get('-') ?? 0), COUNT).trim()} of queries (measured) — IS [NOT] NULL, and = <> < <= > >= against a literal`,
)
console.log(
  `         drawn from the column's own seeded values, combined with AND, OR, NOT, AND(OR)`,
)
...
console.log(
  `  join kinds:  ` +
    [...kindUse.entries()]
      .sort((a, b) => b[1] - a[1])
      .map(([k, n]) => `${k.replace('JOIN_', '')} ${pct(n, totalKinds)}`)
      .join('   '),
)
console.log(
  `  expression targets: ` +
...
if (mergeActions.size) {
  ...
  console.log(
    `  MERGE arms that produced a row: ` +
      [...mergeActions.entries()]
        .sort((a, b) => b[1] - a[1])
        .map(([k, n]) => `${k} ${pct(n, tot)}`)
        .join('   '),
  )
...
if (paramQueries > 0) {
  ...
  console.log(
    `  ${paramQueries} queries carried ${paramTotal} parameters; ${variantRuns} NULL-binding variants ran`,
  )
  console.log(
    `  sites: ` +
      [...paramSiteUse.entries()]
        .sort((a, b) => b[1] - a[1])
        .map(([k, n]) => `${k} ${n}`)
        .join('   '),
  )
  console.log(
    `  notNull claims: ${pWitnessed} witnessed, ${pUnwitnessed} unwitnessed in this state` +
      (pRaisedOther ? `, ${pRaisedOther} raised outside the enumerated rejection list` : '') +
      `; nullable: ${pNullable}`,
  )
  if (setsSeen) {
    console.log(
      `  joint rejection sets: ${setsSeen} claimed — ${setsWitnessed} witnessed, ` +
...
if (findings.length > 0) {
  ...
  for (const [b, m] of byBucket) {
    ...
    console.log(
      `    (a construct near 100% here and rare in the run is the suspect;` +
